...
- Split stats into two groups: Core Stats (Power / Players / CH25) and
  Additional Stats (KP / Kills / Deads).
- Simplified indicators: up arrow "↑", down arrow "↓", and "–" for same/unknown.
  Indicator is separated from numbers with spaces to avoid squashed appearance.
...
def build_kingdom_summary_embed(
    ...
    *,
    ...
    d_rank = compute_delta(rank, prev_rank)  # negative => improved
    ...
    try:
        ...
        rank_display = "—"
    rank_ind = _indicator(d_rank, invert_for_rank=True)
    rank_delta = (
    ...
    add_lines = []
    add_lines.append(
        f"**Total Kill Points (KP):** {_fmt_short_safe(kp)}  {_indicator(d_kp)}  {_fmt_signed_short(d_kp)}"
    )
    add_lines.append(
        f"**Total T4 & T5 Kills:** {_fmt_short_safe(kills)}  {_indicator(d_kills)}  {_fmt_signed_short(d_kills)}"
    )
    add_lines.append(

...
from stats_alerts.allkingdoms import load_kingdom_summary
from stats_alerts.formatters import abbr, fmt_honor
...
async def send_prekvk_embed(
    ...
    *,
    ...
    # Kingdom summary — load KS table (off-thread)
    ks = None
    try:
        try:
            from file_utils import run_blocking_in_thread
        ...
        if run_blocking_in_thread is not None:
            ks = await run_blocking_in_thread(
                load_kingdom_summary,
                name="load_kingdom_summary",
                meta={"caller": "stats_alerts.embeds.prekvk.send_prekvk_embed"},
            )
        else:
            ks = await asyncio.to_thread(load_kingdom_summary)
    except Exception:
        logger.exception("[PREKVK] Failed to load kingdom summary (KS)")

    if ks:
        # Format the kingdom summary in a compact block (reuse abbr for compact numbers)
        try:
            kp = ks.get("KP") if "KP" in ks else ks.get("kp") or ks.get("KP")
            power = ks.get("KINGDOM_POWER") or ks.get("Kingdom_Power") or ks.get("KINGDOMPOWER")
            governors = ks.get("Governors") or ks.get("Governors")
            deads = ks.get("DEAD") or ks.get("Dead") or ks.get("DEAD")
            krank = ks.get("KINGDOM_RANK") or ks.get("KINGDOM_RANK")
            kseed = ks.get("KINGDOM_SEED") or ks.get("KINGDOM_SEED")

            ks_lines = [
                f"**Total Power:** {abbr(power)}",
                f"**Governors:** {int(governors or 0)}",
                f"**KP:** {abbr(kp)}",
                f"**Total Deads:** {abbr(deads)}",
                f"**Kingdom Rank:** {krank if krank is not None else '—'}",
                f"**Seed:** {kseed if kseed is not None else '—'}",
            ]
            embed.add_field(name="👑 Kingdom Summary", value="\n".join(ks_lines), inline=False)
        except Exception:
            logger.exception("[PREKVK] Failed building KS summary field")

    ...
    try:
        ...
        if run_blocking_in_thread is not None:
            tops = await run_blocking_in_thread(
                ...
                int(kvk_no) if str(kvk_no).isdigit() else 0,
                name="load_allkingdom_blocks",
                meta={"kvk_no": kvk_no},
            )
        else:
            logger.debug(
                "[PREKVK] run_blocking_in_thread not available; using asyncio.to_thread fallback for load_allkingdom_blocks (consider converting to run_blocking_in_thread)"
            )

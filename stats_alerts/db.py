...
def run_query_strict(
    sql: str, params: Sequence[Any] | None = None, *, timeout_ms: int | None = None
) -> list[dict]:
    """
    Run a query and return list[dict] rows.

    Unlike run_query(), this helper does not swallow exceptions.
    """
    conn = get_conn_with_retries()
    try:
        cur = conn.cursor()
...
def run_one_strict(sql: str, params: Sequence[Any] | None = None) -> dict | None:
    """
    Execute a query and return the first row as a dict, or None.

    Unlike run_one(), this helper does not swallow exceptions. Use for
    critical mutation/read paths where callers must differentiate SQL errors
    from "no row found".
    """
    conn = get_conn_with_retries()
    with conn:
...
async def run_one_strict_async(sql: str, params: Sequence[Any] | None = None) -> dict | None:
    try:
        from file_utils import run_blocking_in_thread

        return await run_blocking_in_thread(
            run_one_strict, sql, params, name="db_run_one_strict", meta={"sql": sql[:256]}
        )
    except Exception:
        return await asyncio.to_thread(run_one_strict, sql, params)


async def run_query_strict_async(
    sql: str, params: Sequence[Any] | None = None, *, timeout_ms: int | None = None
) -> list[dict]:
    try:
        from file_utils import run_blocking_in_thread
        ...
        return await run_blocking_in_thread(
            ...
            params,
            timeout_ms=timeout_ms,
            name="db_run_query_strict",
            meta={"sql": sql[:256]},
        )
    except Exception:
        return await asyncio.to_thread(run_query_strict, sql, params, timeout_ms=timeout_ms)

...
async def get_latest_honor_top(n: int = 3) -> list[dict[str, Any]]:
    ...
    Runs in a thread via run_query_async (which uses asyncio.to_thread internally).
    """
    # Validate n
    try:
        n_i = int(n)
...
def purge_latest_honor_scan() -> int:
    ...
    This is synchronous (disk/DB) and should be called via asyncio.to_thread if used in async code.
    """
    try:

...
def build_target_embed(data):
    ...
    if last_kvk:
        try:
            ...
            lk_dkp = float(last_kvk.get("DKP Score", 0) or 0.0)
            lk_pct = None
            if lk_kill_target:
                lk_pct = (lk_total_kills / lk_kill_target) * 100.0
            lk_line = (
                f"KVK {lk_kvk_no}: Kills {fmt_short(lk_total_kills)} / {fmt_short(lk_kill_target)} "
                + (f"({fmt_pct(lk_pct)}) " if lk_pct is not None else "")
...
def build_stats_embed(governor_data, discord_user) -> tuple[discord.Embed, discord.File]:
    ...
    dkp_target = _get_int_from_variants(
        governor_data, ["DKP Target", "DKP_Target", "DKPTarget", "DKP_TARGET"], default=0
    )
    ...
    # New: Last KVK summary field (try governor_data['last_kvk'] first, then fallback to file)
    last_kvk = governor_data.get("last_kvk") or _load_last_kvk_for_governor(str(governor_id))
    if last_kvk:
        try:
            # extract common values defensively
            lk_kvk_no = last_kvk.get("KVK_NO", None)
            lk_total_kills = int(last_kvk.get("T4&T5_Kills", 0) or 0)
            lk_kill_target = int(last_kvk.get("Kill Target", 0) or 0)
            lk_dkp = float(last_kvk.get("DKP Score", 0) or 0.0)

            # attempt to read DKP target from a few common variants
            try:
                lk_dkp_target = int(
            ...
            # compute percentages only if target exists
            lk_kill_pct = None
            if lk_kill_target:
            ...
            lk_dkp_pct = None
            if lk_dkp_target:
                lk_dkp_pct = (lk_dkp / lk_dkp_target) * 100.0

            # Build two-line value to match /mykvktargets formatting
            kills_line = f"⚔️ Kills {fmt_short(lk_total_kills)} / {fmt_short(lk_kill_target)}"
            if lk_kill_pct is not None:
                kills_line += f" ({fmt_pct(lk_kill_pct)})"

            # DKP line: include target/percent only when available
            if lk_dkp_target:
                dkp_line = f"🎯 DKP {fmt_short(int(lk_dkp))} / {fmt_short(int(lk_dkp_target))} ({fmt_pct(lk_dkp_pct)})"
            else:
                dkp_line = f"🎯 DKP {fmt_short(int(lk_dkp))}"

            field_name = f"Last KVK Stats • KVK: {lk_kvk_no}"
            field_value = f"{kills_line}\n{dkp_line}"

            embed.add_field(name=field_name, value=field_value, inline=False)
        except Exception:
            logger.exception("[EMBED] Failed to render last_kvk summary for stats embed")

...
try:
    from embed_utils import brand_color, format_number_short
except Exception:
...
def build_kvk_targets_embed(
    ...
    state = (targets.get("TargetState") or "ACTIVE").upper()
    kvk_no = targets.get("KVK_NO")

    dkp = targets.get("DKP_Target")
    kills = targets.get("Kill_Target")
    deads = targets.get("Deads_Target")
    min_k = targets.get("Min_Kill_Target")
    power = targets.get("Power")

    # Title = big name/id
    title = f"{gov_name} • {governor_id}"
    ...
    em.set_footer(text=f"GovernorID: {governor_id} • {foot}")

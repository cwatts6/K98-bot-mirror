...
def apply_import_plan(
    ...
    *,
    ...
    """
    Returns (registry_dict, summary_lines, error_lines).
    In dry-run mode errors is always [].
    In live mode errors contains rows that were skipped or failed.
    """
    """
    Apply a validated import plan to the registry.

    Dry-run mode:
        Builds an in-memory preview dict from existing_registry + changes.
        existing_registry must be provided (loaded from SQL by the caller).
        No SQL writes performed.

    Live mode:
        Writes each change to SQL via registry_dal.upsert_from_import()
        using Overwrite conflict behaviour existing active slots for the
        same (DiscordUserID, AccountType) are superseded and replaced.
        existing_registry is not used in the live path; SQL constraints
        enforce all uniqueness rules atomically.
        Returns the post-import state loaded fresh from SQL.

    Returns (registry_dict, summary_lines).
    """
    ...
    # Live path — existing_registry not used; SP enforces constraints.

    errors: list[str] = []

    for ch in changes:
        did_str = str(ch["discord_id"])
        acct = ch["account_type"]
        gid_raw = ch["governor_id"]
        gname = ch.get("governor_name", "") or ""

        try:
            did_int = int(did_str)
        except ValueError:
            errors.append(f"Skipped {did_str} {acct}: invalid Discord ID")
            continue

        try:
            gid_int = int(str(gid_raw).strip())
        except (ValueError, TypeError):
            errors.append(f"Skipped {did_str} {acct}: invalid GovernorID {gid_raw!r}")
            continue

        code, msg = registry_dal.upsert_from_import(
            discord_user_id=did_int,
            discord_name=None,
            governor_id=gid_int,
            governor_name=gname or None,
            account_type=acct,
            provenance="import",
        ...
        if code in (0, 5):
            summary.append(f"{did_str}: {acct} -> {gid_raw} ({gname})")
        elif code == 2:
            errors.append(
                f"Row {ch.get('source_row', '?')}: "
                f"GovernorID {gid_raw} already registered to another user — skipped."
            )
        else:
            errors.append(f"Row {ch.get('source_row', '?')}: {msg}")

    logger.info(
        "[IMPORT] apply_import_plan complete: %d applied, %d errors",
        len(summary),
        len(errors),
    )
    for e in errors:

...
## Update — Admin Add Governor Lookup UX
✅ Updated

- Admin Add modal now supports **Governor Name or GovernorID** input.
- Resolution flow supports:
  - Direct numeric GovernorID path
  - Name lookup path via existing lookup flow
  - Fuzzy disambiguation selector when multiple matches are returned
- Behavior now aligns with existing `/player_profile`-style lookup ergonomics.

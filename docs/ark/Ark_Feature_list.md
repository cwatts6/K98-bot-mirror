...
### ✅ Phase 8 — Bans with “Next N Ark Weekends” Expiry (Implemented)

Task 8A — Ban rules + auto expiry

**Delivered**
- Ban model supports target by:
  - `discord_user_id` and/or
  - `governor_id`
- Ban add supports:
  - `banned_ark_weekends` (N)
  - `reason`
- Ban start policy:
  - Starts from the **next Ark weekend on/after now UTC** (from ArkConfig cadence).
- Revoke vs expiry:
  - Explicit revoke supported (`RevokedAtUtc`, `RevokedByDiscordId`)
  - Natural expiry supported by weekend-window coverage.
- Conflict rule:
  - Multiple ban rows are allowed.
  - Enforcement blocks if **any active ban** applies.
- Enforcement points:
  - Self signup blocked by active ban.
  - Admin add blocked by active ban by default.
  - Admin emergency override supported via `AdminOverrideBanRule` (default OFF).
- Messaging/logging:
  - Shared/standardized user-facing ban block messages.
...
✅ Revoke immediately removes block for future signups  
✅ Self signup + admin add enforcement verified  
✅ Clear user/admin-facing errors in place  

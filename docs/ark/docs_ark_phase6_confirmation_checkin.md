...
/* =========================================
   Ark Match Reset + Reseed Script (additive signups)
   ========================================= */

DECLARE @MatchId bigint = 13;  -- << set match id to reset
DECLARE @Alliance nchar(255) = N'Hlks'; -- << set alliance
DECLARE @ArkWeekendDate date = '2026-03-05'; -- << set test weekend date
DECLARE @MatchDay char(3) = 'Sat'; -- Sat or Sun
DECLARE @MatchTimeUtc time(0) = '10:45'; -- << set test time
...
DECLARE @PlayerDiscordId bigint = 559076207627468807; 

DECLARE @SubGovernorId bigint = 2510418;
DECLARE @SubName nvarchar(64) = N'Scrooge M';
DECLARE @SubDiscordId bigint = 559076207627468807; --same discordid to simplify for testing

-- 1) Clean audit log ONLY (optional)
DELETE FROM dbo.ArkAuditLog WHERE MatchId = @MatchId;

-- 2) Upsert match row (do not delete signups)
IF EXISTS (SELECT 1 FROM dbo.ArkMatches WHERE MatchId = @MatchId)
BEGIN
    UPDATE dbo.ArkMatches
    SET Alliance = @Alliance,
        ArkWeekendDate = @ArkWeekendDate,
        MatchDay = @MatchDay,
        MatchTimeUtc = @MatchTimeUtc,
        SignupCloseUtc = @SignupCloseUtc,
		Status = 'Scheduled',
...
-- 3) Seed signups ONLY if they don't already exist
IF NOT EXISTS (
    SELECT 1 FROM dbo.ArkSignups WHERE MatchId = @MatchId AND GovernorId = @PlayerGovernorId
)
BEGIN
...
IF NOT EXISTS (
    SELECT 1 FROM dbo.ArkSignups WHERE MatchId = @MatchId AND GovernorId = @SubGovernorId
)
BEGIN
    INSERT INTO dbo.ArkSignups
        (MatchId, GovernorId, GovernorNameSnapshot, DiscordUserId, SlotType, Status,
...
-- 4) Optional: lock immediately (if you want confirmation embed to post right away)
-- UPDATE dbo.ArkMatches

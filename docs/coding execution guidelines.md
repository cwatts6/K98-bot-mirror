...
JSON-backed state files (ark_reminder_state.json, ark_team_state.json) are supplementary caches only. 
Any reminder deduplication that relies solely on these files must be tolerant of file loss on restart. 
SQL-persisted flags (e.g. AnnouncementSent, RegistrationMessageId, CompletionPosted) are authoritative and take precedence.

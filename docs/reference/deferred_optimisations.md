...
- Type: consistency
- Description: Phase 5C implements the Accounts slice with its approved no-avatar 1702x924 standalone card, same-payload fallback, blue-primary navigation, unchanged Manage flow, complete private Account Summary/CSV, and attachment cleanup; operator Discord smoke remains. Reminders, Preferences, Inventory, and Exports still need their separately approved standalone delivery and attachment-lifecycle alignment, while their Discord-user/all-governor semantics mean they must not display a misleading Change Governor dropdown.
- Suggested Fix: Finish Phase 5C operator smoke, then deliver four independent asset-led slices: Phase 5D Reminders, Phase 5E Preferences, Phase 5F Inventory summary, and Phase 5G Exports summary. For each page, retain the current 1702x924 output and stable filename unless a page-specific operator checkpoint explicitly approves different dimensions, migrate successful generated cards from embed-wrapped `attachment://` presentation to standalone attachments, retain the existing private same-payload fallback, preserve page actions and disabled states, apply the blue primary top-row pattern consistently, use only that page's approved avatar/no-avatar treatment, retain selected governor context only for returning to governor-specific pages, and add transition/fallback/timeout cleanup plus desktop/mobile smoke coverage. Do not add Change Governor to these Discord-user/all-governor pages or create a broad shared framework. Governor-specific child actions must resolve their governor explicitly and recheck access.
- Impact: medium
- Risk: medium
- Dependencies: Phase 4 operator acceptance on 2026-07-11; Phase 5A operator acceptance on 2026-07-13; Phase 5B Inventory report visual alignment and final operator acceptance completed on 2026-07-13; Phase 5C Accounts implementation complete with operator smoke pending; operator-supplied/approved page-specific assets and separate Phase 5D-5G scope/visual approvals; preserve existing Reminders, Preferences, Inventory, and Exports semantics.

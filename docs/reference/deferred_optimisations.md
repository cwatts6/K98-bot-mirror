...
### Deferred Optimisation
- Area: `player_self_service/preference_service.py`, `player_self_service/service.py`, `ui/views/player_self_service_views.py`, SQL repo `C:\K98-bot-SQL-Server`
- Type: architecture
- Description: Timezone, location country, and preferred language would be valuable for future event planning and localization, but they are Discord-user-level settings rather than governor-level settings. The existing `dbo.DiscordGovernorRegistry` stores governor registration slots, so adding these values there would duplicate user-level data across Main/Alt/Farm rows and risk drift between rows for the same Discord user.
- Suggested Fix: Scope Phase 12B to add a dedicated SQL-backed Discord user preference/profile store keyed by Discord user ID, with nullable timezone, location country, and preferred language fields, service/DAL accessors, validation, card display, and Manage add/update/remove flows. Preserve the current session-based local-time toggle and do not apply saved timezone to display behavior until a separately approved planning feature uses it.
- Impact: high
- Risk: medium
- Dependencies: Requires explicit SQL schema approval, K98 SQL repo migration, timezone/country/language validation choices, privacy copy approval, focused persistence and restart-safety tests, and `k98-sql-validation`.

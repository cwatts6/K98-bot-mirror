...
- Type: consistency
- Description: Phase 5C completed the Accounts slice with its accepted avatar-enabled 1702x924 standalone card, same-payload fallback, page-relevant blue navigation, unchanged Manage flow, complete private Account Summary/CSV, graceful timeout, and attachment cleanup. Reminders, Preferences, Inventory, and Exports still need their separately approved standalone delivery and attachment-lifecycle alignment, while their Discord-user/all-governor semantics mean they must not display a misleading Change Governor dropdown.
- Suggested Fix: Deliver four independent product-workshopped slices: Phase 5D Reminders, Phase 5E Preferences, Phase 5F Inventory summary, and Phase 5G Exports summary. For each page, start from the accepted 1702x924 standalone/avatar/same-payload-fallback/graceful-timeout baseline and stable filename unless a page-specific operator checkpoint approves another treatment; preserve genuine page actions and disabled states; approve page-relevant component rows rather than mechanically copying irrelevant navigation; retain selected governor context only for returning to governor-specific pages; and add transition/fallback/timeout cleanup plus original/desktop/mobile smoke coverage. Do not add Change Governor to these Discord-user/all-governor pages or create a broad shared framework. Governor-specific child actions such as Preferences VIP and Inventory Open Report must resolve their governor explicitly, recheck access, and use the accepted paged selector/report-switching contract where applicable.
- Impact: medium
- Risk: medium
- Dependencies: Phase 4 operator acceptance on 2026-07-11; Phase 5A operator acceptance on 2026-07-13; Phase 5B Inventory report visual alignment and final operator acceptance completed on 2026-07-13; Phase 5C Accounts final operator acceptance completed on 2026-07-14; active Phase 5D workshop pack; operator-approved page-specific assets/contracts and separate Phase 5D-5G implementation approvals; preserve existing Reminders, Preferences, Inventory, and Exports semantics.

...
- Type: cleanup
- Description: Phase 5F removes the final approved player-facing need for Inventory report visibility by retiring `/myinventory`, `/inventory_preferences`, public Inventory posting, and the Personal Settings Privacy & Sharing control. The application should stop reading and writing the preference, but the SQL table and existing rows are intentionally retained during the first release as rollback evidence. Dropping the table in the bot cleanup would create an unnecessary irreversible SQL dependency and would exceed the no-SQL-deployment Phase 5F boundary.
...
- Type: architecture
- Description: GovernorOS v2 Phases 1-5E are complete and operator accepted. Phase 5F is now an active task-packed Inventory Surface Consolidation and Legacy Retirement slice rather than an asset-led Inventory summary. It coordinates removal of `/me inventory`, `/myinventory`, `/inventory_preferences`, the obsolete Inventory-visibility setting, and directly orphaned code/assets while preserving the selected-governor dashboard, premium private Inventory reports, imports, audits, and exports. The remaining roadmap is Phase 5G Exports summary; Phase 6 Export Stats integration after a selected-governor versus all-linked decision; Phase 7 private `/me history`; Phase 8 permission-gated admin/leadership `/me inspect`; and Phase 9 usage-led review of still-retained compatibility paths. Phase 10 sticky features remain a future programme candidate.
- Suggested Fix: Execute the approved Phase 5F pack first as one coordinated bot release, then continue Phases 5G-9 as separate task-packed slices in the authoritative order. Preserve the Phase 2 payload/privacy boundary, Phase 3 access/selector journey, Phase 4 standalone delivery/navigation contract, Phase 5A direct-report controls, Phase 5B premium Inventory report contract, Phase 5C portfolio/Account Summary contract, Phase 5D/5D.1 reminder contract, and Phase 5E regional-profile/local-time plus Accounts-owned VIP contract. Phase 5F explicitly supersedes only Phase 5E's Inventory-privacy ownership; it does not approve broad renderer consolidation, export redesign, History, Inspect, or unrelated legacy removals.
- Impact: high
- Risk: medium
- Dependencies: Phases 1-5E delivered and accepted through production PR #531 on 2026-07-16; Phase 5F usage evidence and operator approval recorded; explicit approval remains required for every later phase gate.
...
- Type: consistency
- Description: Phase 5C completed Accounts, Phase 5D/5D.1 completed Reminders, and Phase 5E completed Preferences plus the Accounts-owned Update VIP migration. Product evidence subsequently superseded the proposed Inventory summary alignment: Phase 5F is approved to remove the obsolete Inventory summary/navigation path and simplify Personal Settings. After that work, Exports is the only remaining `/me` summary page awaiting dedicated standalone delivery and attachment-lifecycle alignment.
- Suggested Fix: In Phase 5F, remove only the Inventory-specific generic page path and preserve the proven modern selected-governor report lifecycle. Then deliver Phase 5G Exports as its own product-workshopped slice using the accepted standalone/private/same-payload-fallback/graceful-timeout contract. Keep selected governor context only for Dashboard return, show no Change Governor on Exports, and do not create a broad shared framework in either slice.
- Impact: medium
- Risk: medium
- Dependencies: Phases 4-5E accepted through 2026-07-16; active Phase 5F consolidation pack; future operator approval and acceptance of Phase 5G Exports.

### Deferred Optimisation
- Area: `player_self_service/page_cards.py`, page-specific `/me` renderers, and shared attachment/view lifecycle helpers
- Type: architecture
- Description: GovernorOS intentionally delivers Dashboard, Accounts, Reminders, and Preferences through dedicated payload/renderer paths while Inventory summary and Exports still enter the older generic page-card renderer. Phase 5F is approved to remove the Inventory caller and its page-specific branches/assets, but not to turn that deletion into a broad renderer/view framework. After Phase 5F, Exports should be the only supported runtime summary-page caller of the generic card path; historical mappings or helpers may also become mechanically dead and must be proven before deletion.
- Suggested Fix: Let Phase 5F remove only Inventory-specific generic-card code that is directly orphaned. After Phase 5G gives Exports an accepted replacement, inventory the remaining duplicated rendering, attachment, fallback, navigation, and timeout primitives. Consolidate only helpers with identical proven contracts; keep page payloads, product copy, geometry, and business ownership page-specific. Remove the generic card path only when zero supported callers remain and focused lifecycle regression coverage exists.
- Impact: low
- Risk: medium
- Dependencies: Phase 5E deployment on 2026-07-16; active Phase 5F retirement; future Phase 5G acceptance; measured zero-caller evidence before final generic-renderer removal.

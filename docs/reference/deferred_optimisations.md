...
- Type: architecture
- Description: GovernorOS v2 Phases 1-5C are complete and operator accepted. The remaining committed roadmap is separate asset-led Phases 5D-5G for Reminders, Preferences, Inventory summary, and Exports summary; Phase 6 Export Stats integration after a selected-governor versus all-linked decision; Phase 7 private `/me history`; Phase 8 required permission-gated admin/leadership `/me inspect`; and Phase 9 usage-led migration review. Phase 10 sticky features are a future programme candidate rather than committed implementation. Each remaining phase has a distinct renderer, page, privacy, compatibility, or product-decision boundary and must not be collapsed into one broad PR.
- Suggested Fix: Start with the Phase 5D Reminders product/visual workshop, then execute Phases 5D-9 as separate task-packed slices in the authoritative programme order. Preserve the Phase 2 payload/privacy boundary, Phase 3 access/selector journey, Phase 4 standalone delivery/navigation contract, Phase 5A direct-report controls, Phase 5B premium Inventory report contract, and Phase 5C all-linked portfolio/Account Summary contract. Apply the fixed governor-routing matrix: no Change Governor on Discord-user/all-linked summary pages; explicit current-access-checked selection on governor-specific pages/child actions. Require the documented operator checkpoint for each remaining page visual and navigation contract, Phase 6 export scope, Phase 7 history controls, Phase 8 permissions/VIP/lookup/telemetry, and every Phase 9 migration decision. Create a new successor pack before any Phase 10 implementation.
- Impact: high
- Risk: medium
- Dependencies: Phase 1 blueprint archived; Phase 2 delivered in mirror PR #216 and production PR #523; Phase 3 delivered in mirror PR #217 and production PR #524 with automated validation and operator Discord smoke completed on 2026-07-10; Phase 4 delivered in mirror PR #218 with operator Discord smoke completed on 2026-07-11; Phase 5A delivered in mirror PR #219 and production PR #526 with operator smoke completed on 2026-07-13; Phase 5B delivered in mirror PR #220 and production PR #527 with operator smoke/final visual acceptance completed on 2026-07-13; Phase 5C delivered in mirror PR #221 and production PR #528 with final operator smoke/premium visual acceptance completed on 2026-07-14; explicit approval for each later phase gate.
...
- Type: consistency
- Description: Phase 5C completed Accounts and Phase 5D completed Reminders. Phase 5E implements the approved Preferences standalone card and moves governor-specific VIP editing to Manage Accounts. Inventory summary and Exports still need their separately approved standalone delivery and attachment-lifecycle alignment, while these Discord-user/all-governor pages must not display a misleading Change Governor dropdown.
- Suggested Fix: Complete Phase 5E validation/operator acceptance, then deliver Phase 5F Inventory summary and Phase 5G Exports summary as independent product-workshopped slices. For each page, start from the accepted 1702x924 standalone/avatar/same-payload-fallback/graceful-timeout baseline and stable filename unless a page-specific checkpoint approves another treatment; preserve genuine page actions and disabled states; approve page-relevant component rows; retain selected governor context only for returning to governor-specific pages; and add transition/fallback/timeout cleanup plus original/desktop/mobile smoke coverage. Do not add Change Governor or create a broad shared framework. Governor-specific Inventory Open Report must keep explicit selection, access recheck, and the accepted paged selector/report-switching contract.
- Impact: medium
- Risk: medium
- Dependencies: Phase 4 operator acceptance on 2026-07-11; Phase 5A operator acceptance on 2026-07-13; Phase 5B acceptance on 2026-07-13; Phase 5C acceptance on 2026-07-14; Phase 5D/5D.1 acceptance on 2026-07-15; Phase 5E validation/operator acceptance; operator-approved Phase 5F-5G assets/contracts; preserve existing Inventory and Exports semantics.

### Deferred Optimisation
- Area: `player_self_service/page_cards.py`, page-specific `/me` renderers, and shared attachment/view lifecycle helpers
- Type: architecture
- Description: GovernorOS intentionally delivers Accounts, Reminders, and Preferences through narrow page-owned payloads/renderers while legacy Dashboard, Inventory summary, and Exports still share the older generic page-card path. Phase 5E found no safe reason to introduce a broad renderer/view framework before the remaining page contracts are known. The earlier cross-field profile write-concurrency concern is not deferred: Phase 5E resolves it with an atomic field-specific DAL upsert.
- Suggested Fix: After Phases 5F and 5G are accepted, inventory the remaining duplicated rendering, attachment, fallback, navigation, and timeout primitives. Consolidate only helpers with identical proven contracts; keep page payloads, product copy, geometry, and business ownership page-specific. Remove the legacy generic card path only when every remaining caller has an accepted replacement and focused lifecycle regression coverage.
- Impact: low
- Risk: medium
- Dependencies: Operator acceptance of Phases 5E-5G; measured duplication across the final page-specific implementations; no product or persistence changes in the consolidation slice.

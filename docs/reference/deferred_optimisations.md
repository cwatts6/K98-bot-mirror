...
- Area: Inventory upload route/view/service workflow
- Type: consistency
- Description: Task C Slice 3 mapped the non-fallback import state surfaces, Task C Slice 3A normalized batch-level `RowsInSource`, Task C Slice 4 delivered Honor generic audit adoption, Task C Slice 5 delivered PreKvK generic audit adoption, Task C Slice 6 delivered weekly activity generic audit adoption, and Task C Slice 7 delivered MGE results generic audit adoption. Inventory remains the final import-family generic audit adoption slice, and it needs a deliberate slice because it has a long-lived route/view/service workflow with image analysis, command-session handoff, multi-governor selection, correction, additional-material continuation, approval, rejection, cancellation, timeout, admin debug, original-upload cleanup, and an existing inventory-specific domain audit/history model.
- Suggested Fix: Continue with Task C Slice 8 for inventory generic correlation. Validate `dbo.InventoryImportBatch.ImportBatchID` as the external correlation candidate, map pre-batch, analysed, approved, rejected, cancelled, failed, timeout, and additional-material outcomes carefully, and do not replace inventory's domain audit/history model. Preserve route/command UX, image-analysis behavior, SQL table behavior, report/export output, admin debug behavior, original-upload cleanup, and existing worker recovery semantics unless separately approved.
- Impact: medium
- Risk: medium
- Dependencies: Generic durable import batch audit foundation delivered and smoke tested in Task C Slice 2; player-location adoption delivered and smoke tested in Task C Slice 3; batch-level `RowsInSource` normalization delivered and smoke tested in Task C Slice 3A; Honor adoption delivered and smoke tested in Task C Slice 4; PreKvK adoption delivered and smoke tested in Task C Slice 5 with accepted, duplicate, and rejected outcomes; weekly activity adoption delivered and smoke tested in Task C Slice 6; MGE results adoption delivered and reported smoke tested in Task C Slice 7; inventory import-kind-specific tests identified for route/view/service/DAL/domain-audit paths.

### Deferred Optimisation
- Area: `ui/views/inventory_views.py`, `inventory/inventory_service.py`, inventory import lifecycle callbacks
- Type: architecture
- Description: Inventory import lifecycle coordination remains intentionally view-heavy. `ui/views/inventory_views.py` routes upload-first messages, command-session continuations, multi-governor selection, review interactions, correction modals, additional-material continuation, approval, rejection, cancellation, timeout, admin-debug posting, and original-upload cleanup. Task C Slice 8 should adopt generic audit without redesigning this workflow, but the audit pass may identify lifecycle coordination that would be cleaner behind narrower service helpers after the audit contract is stable.
- Suggested Fix: After Task C Slice 8 generic audit adoption is delivered and smoke tested, audit inventory import view callbacks for service-extraction opportunities. Move only stable lifecycle transitions, admin-debug/reference updates, material-continuation coordination, and original-upload cleanup orchestration behind service helpers with focused interaction tests, preserving `/inventory import`, upload-first, correction, materials, admin debug, and user-facing behavior.
- Impact: medium
- Risk: medium
- Dependencies: Task C Slice 8 inventory generic audit adoption and smoke evidence; existing inventory route/view/service/DAL tests; no command UX or SQL schema changes without a separate approved slice.

# K98 Bot Deferred Optimisation Scoring Model

> Canonical repo copy: `docs/reference/K98 Bot Deferred Optimisation Scoring Model.md`.

## 1. Purpose

Use this model to prioritise deferred optimisation items consistently when preparing an
optimisation batch, GitHub issue set, or Codex task pack.

This model is a decision aid, not a substitute for judgement. It is not required reading for every
ordinary repo task.

## 2. Scoring Fields

Each candidate item receives four scores from 1 to 5.
...
|-------|----------------|
| 10 to 14 | Prioritise soon |
| 7 to 9 | Good batch candidate |
| 4 to 6 | Keep in backlog |
| 1 to 3 | Low priority |
| 0 or below | Do not action unless bundled with related work |
...
- improve testability
- reduce production support risk

## 5. Batch Selection Rules

A good optimisation batch should usually include:

- 3 to 7 related items
- one clear subsystem or theme
...
When preparing a Deferred Optimisation to Codex task pack:

- score each candidate item
- explain exclusions
- group related items
- avoid relying on score alone

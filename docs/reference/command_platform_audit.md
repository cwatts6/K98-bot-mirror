...
Current command-surface note (2026-07-19): GovernorOS v2 Phase 6 removed `/my_stats` while adding
grouped `/me stats`; Phase 7 then completed the visual/content closeout with no command change or
resync. The maintained validator/canonical baseline is now
`primary=37 grouped_subcommands_detected=100 total_unique=37`, with eight `/me` and two
`/inventory` subcommands. The Phase 5A baseline below remains the historical programme audit
baseline.
...
| Inventory | Service-oriented implementation and focused inventory tests. Phase 5A grouped import/audit admin paths under `/inventory`; GovernorOS v2 Phase 5F later retired the combined Inventory export and `/export_inventory`. | Retained selected-governor report-page exports recheck self-service access; personal commands remain high-discoverability. | Keep broader personal export redesign in its separately approved GovernorOS/Exports scope. |
...
- Resolution: Added reusable decorator support for admin-only, admin-or-leadership in allowed channels, and missing-config channel denial. Preserved the command paths, command count, service handoff behavior, and denial visibility that existed at Phase 1 completion. At that time `/export_inventory` remained service-authorized rather than decorator-denied; GovernorOS v2 Phase 5F later retired that command and combined export after separate usage evidence and operator approval.

...
| Player Self-Service | `/me reminders` | `commands/me_cmds.py` | Grouped | Public command-level access | Ephemeral | Standard (`v1.01`) | Phase 5D complete and operator accepted 2026-07-15; preserve premium summary and existing Manage behavior | Private Discord-user-level premium reminder centre with invoking-user avatar/safe fallback, duplicate-safe Kingdom identity, earned ACTIVE/REVIEW/OFF state, approved REMINDER COVERAGE hero, friendly KVK/Calendar event and alert-time summaries, one deterministic insight, standalone `me_reminders_<discord_user_id>.png` delivery, and same-payload fallback. The existing Manage journey retains KVK autosave/remove-all/confirmation DM and Calendar Settings behavior. It has no Inventory navigation, Change Governor, or selected-governor filter; optional Dashboard governor context is return-only. Phase 13 redirects `/subscribe`, `/modify_subscription`, `/unsubscribe`, and `/calendar_reminder_config` here. Scheduler, persistence, retry, duplicate-send, DM, event-source, calendar, and SQL behavior are unchanged. Final operator smoke passed Manage refresh/reflected updates, timeout, avatar/identity, navigation, alignment, and dated-footer presentation. Phase 5D.1 is the separately scoped authoritative next-alert hero completion. |
...
- GovernorOS v2 Phase 5A completed private `/me resources`, `/me materials`, and `/me speedups`
  ...
  is the next active slice: it will reuse the occurrence-reader paths behind
  `/calendar_next_event`, `/next_kvk_event`, and `/next_kvk_fight` while extracting shared pure
  scheduler-parity candidates for the authoritative next-alert hero; no command change is implied.

...
[S10E task pack](../task_packs/archive/Codex%20Task%20Pack%20-%20KVK%20Source%20Migration%20S10E%20Export%20Operator%20UX%20and%20Rollover.md).
...
| Stats/KVK | `/kvk_admin test_export` | `commands/stats_cmds.py` | Grouped | Admin notify-channel decorator | Ephemeral | Standard | Preserve; moved from `/kvk test_export` in Phase 2A | Unavailable until coordinated export admission is implemented; rejects before any runner for either source. |
...
| Stats/KVK | `/kvk_admin export_all` | `commands/stats_cmds.py` | Grouped | Admin notify-channel decorator | Ephemeral | Standard | Preserve; moved from `/kvk export_all` in Phase 2A | Unavailable until coordinated export admission is implemented; rejects before any runner for either source. |
| Stats/KVK | `/kvk_admin recompute` | `commands/stats_cmds.py` | Grouped | Admin notify-channel decorator | Ephemeral | Standard | Preserve; moved from `/kvk recompute` in Phase 2A | Fixed-source dispatch: legacy recompute under admission lock; snapshot source inspects only, without writes. |
| Stats/KVK | `/kvk_admin list_scans` | `commands/stats_cmds.py` | Grouped | Admin notify-channel decorator | Ephemeral | Standard | Preserve; moved from `/kvk list_scans` in Phase 2A | Fixed-source scan registry; snapshot includes retained unused logical scans and exact UTC precision. |
...
| Stats/KVK | `/kvk_admin window_preview` | `commands/stats_cmds.py` | Grouped | Admin notify-channel decorator | Ephemeral | Standard | Preserve; moved from `/kvk window_preview` in Phase 2A | Fixed-source private window/complete-selection inspection; pending configuration/counterpart state; fresh metadata/permissions rechecked before response. |

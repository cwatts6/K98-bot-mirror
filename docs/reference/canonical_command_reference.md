...
| Voting Admin | `/vote_admin survey_export` | `commands/vote_admin_cmds.py` | Grouped | Admin or leadership decorator | Ephemeral command response and private CSV file | Standard | Phase 7 first survey slice | Selects one closed survey by autocomplete and privately exports either per-question option totals or response-detail rows via the `mode` option. Response-detail export includes spreadsheet-safe Discord user ID text and resolved Discord name. Free text and Add details columns are deferred to the next phase when those data fields exist. |

...
| Player/KVK | `/kvk history` | `commands/kvk_cmds.py` | Grouped | KVK stats channel decorator | Private picker/error handling where needed; selected/default single-account history posts public; preserves `governor_id` lookup | Standard | Modern Phase 4B card journey complete; legacy `/mykvkhistory` remains live | Player KVK History, Summary, Trends, and CSV export journey. |
| Player/KVK | `/kvk rankings` | `commands/kvk_cmds.py` | Grouped | Mode-specific legacy gates: KVK stats channel with admin override for `kvk`, `prekvk`, and `records`; KVK stats channel without admin override for `honor` | Public unified browser for KVK, Honor, PreKvK, and records | Standard | Phase 5 ranking hub foundation; legacy ranking commands remain live | Required `type` option supports `kvk`, `honor`, `prekvk`, and `records`; current KVK, Honor, PreKvK, and Hall of Fame records Top 10 render visual cards with embed fallback, records remain Top 10 only, and current Top 25/50 remain compact browser output. |
...
| Stats/KVK | `/mykvkhistory` | `commands/stats_cmds.py` | Flat | KVK stats channel decorator with admin override | User-selectable | Standard | Legacy retained during Phase 4B player validation | Personal KVK history chart/table/CSV journey. |

...
| Player Self-Service | `/me exports` | `commands/me_cmds.py` | Grouped | Public command-level access | Ephemeral | Standard | Preserve; Phase 8 private export launchpad | Private personal export launchpad with service-backed default Stats Excel/CSV and Inventory Excel/CSV delivery. Legacy `/my_stats_export` and `/export_inventory` remain live for custom options, and dashboard Quick Launch remains dashboard-only. |
...
- Player Self-Service Command Centre Phase 2 added `/me` with five private subcommands:
  ...
  and inventory export actions while preserving legacy export commands, current export schemas,
  private delivery, and dashboard-only Quick Launch. Existing legacy self-service and calendar

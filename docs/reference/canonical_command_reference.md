...
| Player/KVK | `/kvk rankings` | `commands/kvk_cmds.py` | Grouped | Mode-specific legacy gates: KVK stats channel with admin override for `kvk`, `prekvk`, and `records`; KVK stats channel without admin override for `honor` | Public for KVK/honor/records; PreKvK posts report with private acknowledgement | Standard | Phase 5 ranking hub foundation; legacy ranking commands remain live | Required `type` option supports `kvk`, `honor`, `prekvk`, and `records`. |

...
| Player Self-Service | `/me accounts` | `commands/me_cmds.py` | Grouped | Public command-level access | Ephemeral | Standard | Preserve; GovernorOS v2 Phase 5C premium standalone-card workshop is next | Private Discord-user/all-linked-governor account centre for account review, Governor ID lookup, registration, replacement, and removal with confirmation through one guided Manage journey. It does not show Change Governor; selected governor context is retained only for Dashboard return. |

...
| Player Self-Service | `/me dashboard` | `commands/me_cmds.py` | Grouped | Public command-level access | Ephemeral | Standard | Preserve; Phase 5 visual dashboard card | Private player command centre status dashboard with generated visual card and embed fallback. `/me` owns the cross-domain player portal because account, reminder, preference, export, inventory, and KVK launch guidance do not fit cleanly under one existing domain group. |
| Player Self-Service | `/me accounts` | `commands/me_cmds.py` | Grouped | Public command-level access | Ephemeral | Standard | Preserve; Phase 3 modern account centre smoke tested | Private account centre for account review, Governor ID lookup, registration, replacement, and removal with confirmation. |
| Player Self-Service | `/me reminders` | `commands/me_cmds.py` | Grouped | Public command-level access | Ephemeral | Standard | Preserve; Phase 4 modern reminder centre | Private KVK reminder centre for setup review, subscription, event/time updates, and unsubscribe confirmation. |
| Player Self-Service | `/me preferences` | `commands/me_cmds.py` | Grouped | Public command-level access | Ephemeral | Standard | Preserve; Phase 5 inventory visibility hub | Private player preference status and service-backed inventory visibility controls. |
...
- Player Self-Service Command Centre Phase 2 added `/me` with five private subcommands:
  ...
  visibility controls in `/me preferences`. Existing legacy self-service commands remain

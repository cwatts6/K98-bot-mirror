...
| Player Self-Service | `/me accounts` | `commands/me_cmds.py` | Grouped | Public command-level access | Ephemeral | Standard | Preserve; GovernorOS v2 Phase 5C implemented, operator smoke refinements pending | Private Discord-user/all-linked-governor portfolio card with author avatar, Main/role ordering, latest Kingdom 1198 scan health, Power/T4+T5/current Inventory RSS coverage, deterministic insight, unchanged guided Manage lookup/add/replace/remove, and private paginated Overview/Combat/Economy Account Summary with VIP, KP Loss/Tanking Score, UTC date-times, complete CSV, and graceful disabled-control timeout. Successful visuals are standalone 1702x924 attachments with same-payload fallback. Coordinates and CSV remain private. It does not show Change Governor; selected governor context is retained only for validated Dashboard return. |

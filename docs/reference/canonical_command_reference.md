...
| Player Self-Service | `/me reminders` | `commands/me_cmds.py` | Grouped | Public command-level access | Ephemeral | Standard | Preserve current behavior; GovernorOS v2 Phase 5D product workshop next | Private Discord-user-level reminder centre for KVK event reminder review/autosave/remove-all plus calendar reminder status and service-backed calendar reminder configuration. It has no Change Governor or selected-governor filter. Phase 13 redirects `/subscribe`, `/modify_subscription`, `/unsubscribe`, and `/calendar_reminder_config` here. Phase 5D may modernize its 1702x924 visual/delivery contract only after the operator workshop; reminder scheduling, persistence, DM, event-source, calendar, and SQL changes are not pre-approved. |
...
- GovernorOS v2 Phase 5A completed private `/me resources`, `/me materials`, and `/me speedups`
  ...
  without exposing Change Governor. Phase 5D Reminders is the next product/visual workshop and
  retains the same user-level/no-Change-Governor rule.

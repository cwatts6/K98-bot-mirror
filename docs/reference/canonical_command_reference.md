...
| Player Self-Service | `/me reminders` | `commands/me_cmds.py` | Grouped | Public command-level access | Ephemeral | Standard (`v1.01`) | Preserve Phase 5D premium summary and existing Manage behavior | Private Discord-user-level premium reminder centre with earned ACTIVE/REVIEW/OFF state, approved REMINDER COVERAGE hero, friendly KVK/Calendar event and alert-time summaries, one deterministic insight, standalone `me_reminders_<discord_user_id>.png` delivery, and same-payload fallback. The existing Manage journey retains KVK autosave/remove-all/confirmation DM and Calendar Settings behavior. It has no Change Governor or selected-governor filter; optional Dashboard governor context is return-only. Phase 13 redirects `/subscribe`, `/modify_subscription`, `/unsubscribe`, and `/calendar_reminder_config` here. Scheduler, persistence, retry, duplicate-send, DM, event-source, calendar, and SQL behavior are unchanged. Phase 5D operator Discord smoke remains pending. |
...
- GovernorOS v2 Phase 5A completed private `/me resources`, `/me materials`, and `/me speedups`
  ...
  no-avatar standalone 1702x924 card, earned ACTIVE/REVIEW/OFF state, approved coverage hero,
  ...
  security issue; operator Discord smoke remains pending.

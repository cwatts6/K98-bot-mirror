...
| Player Self-Service | `/me preferences` | `commands/me_cmds.py` | Grouped | Public command-level access | Ephemeral | Standard | Preserve; Phase 5E is the next separately scoped premium summary-card slice | Private Discord-user-level preference status with service-backed inventory visibility controls, access to the existing Governor VIP update flow, and Manage Profile dropdown controls for timezone, location country, and preferred language. Location country is stored as a two-letter code and displayed with a derived readable country name; the private manager child window is replaced after profile updates. Phase 5E must preserve these services and persistence semantics, use the accepted premium private attachment/same-payload fallback lifecycle, and keep the parent page free of `Change Governor`; `Update VIP` remains a governor-specific child journey with explicit governor resolution and a current-access recheck. Its separate task pack/background remain operator-owned and runtime work is not yet approved. |

...
| Player/KVK | `/kvk history` | `commands/kvk_cmds.py` | Grouped | KVK stats channel decorator | User-selectable; Phase 4Biv plans selector removal while preserving `governor_id` lookup | Standard | Modern Phase 4B card journey; legacy `/mykvkhistory` remains live | Player KVK History, Summary, Trends, and CSV export journey. |

...
| Voting Admin | `/vote_admin create` | `commands/vote_admin_cmds.py` | Grouped | Admin or leadership decorator | Ephemeral command response; public vote post side effect | Standard | Phase 5 hidden-until-close results | Creates a SQL-backed live vote post with individual option fields, guided duration choices, up to six vote buttons, result card, reminders, controlled launch mention behaviour, and `result_visibility` selection for public live or hidden-until-close results. Dedicated group approved because there is no `/admin` group and `/ops` is operational rather than voting workflow ownership. |

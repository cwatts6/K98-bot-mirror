...
| Player Self-Service | `/me accounts` | `commands/me_cmds.py` | Grouped | Public command-level access | Ephemeral | Standard | Preserve; Phase 2 read-only shell | Private account setup status and transition guidance. |
...
- Player Self-Service Command Centre Phase 2 added `/me` with five read-only private subcommands:
  `dashboard`, `accounts`, `reminders`, `preferences`, and `exports`. Existing legacy self-service

...
| Player/KVK | `/kvk history` | `commands/kvk_cmds.py` | Grouped | KVK stats channel decorator | Private selector; selected single-account history posts public; preserves `governor_id` lookup | Standard | Modern Phase 4B card journey; legacy `/mykvkhistory` remains live | Player KVK History, Summary, Trends, and CSV export journey. |

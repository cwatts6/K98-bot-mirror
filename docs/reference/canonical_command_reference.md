...
| Player Self-Service | `/me dashboard` | `commands/me_cmds.py` | Grouped | Public command-level access | Ephemeral | Standard | Preserve; Phase 5 visual dashboard card, Phase 7 visual alignment planned | Private player command centre status dashboard with generated visual card and embed fallback. `/me` owns the cross-domain player portal because account, reminder, preference, export, inventory, and KVK launch guidance do not fit cleanly under one existing domain group. |
...
| Player Self-Service | `/me reminders` | `commands/me_cmds.py` | Grouped | Public command-level access | Ephemeral | Standard | Preserve; Phase 6 KVK reminder card and guided Manage flow smoke tested; calendar reminder integration deferred | Private KVK reminder centre for setup review, automatic event/time updates, and remove-all/unsubscribe confirmation. Calendar reminders remain on `/calendar_reminder_config` until the unified reminder phase. |
...
| Player Self-Service | `/me exports` | `commands/me_cmds.py` | Grouped | Public command-level access | Ephemeral | Standard | Preserve; Phase 6 generated card guidance | Private personal export launch guidance; no files are generated by the shell and dashboard Quick Launch remains dashboard-only. |
...
- Player Self-Service Command Centre Phase 2 added `/me` with five private subcommands:
  ...
  preserved `/me exports` as private guidance without dashboard Quick Launch. Calendar reminder
  integration and dashboard visual realignment are deferred to Phase 7. Existing legacy
  self-service commands remain registered in parallel.

...
- Status: `prepared; next KVK Player Experience Redesign phase`
...
Prepare and execute the approved legacy player ranking command transition after the modern
`/kvk rankings` browser has completed rollout validation. Decide, with operator approval, whether
each retained legacy command becomes a redirect/help response, a compatibility shim, or a retired
path, then implement only the approved slice.
...
Legacy player ranking paths intentionally remain live:
...
The legacy KVK and Honor ranking commands still retain older builders/views and duplicated
presentation semantics while `/kvk rankings` uses the shared current-ranking payload/browser
foundation. Legacy `/prekvk report` remains image-based and should not be replaced unless this
phase explicitly receives approval for that user-facing change.
...
- Preserve Honor mode's no-admin-override KVK stats channel gate in both unified and legacy or
  compatibility paths.
- Preserve image-based `/prekvk report` unless an explicit approved Phase 7 decision changes it.
...
- Changing `/kvk stats`, `/kvk targets`, `/kvk history`, or `/kvk_admin` behaviour.
...
- Removing any legacy command before usage review, announcement/redirect planning, and operator
...
- Preserve Honor no-admin-override behaviour.
...
| `/honor_rankings` legacy path duplicates unified ranking semantics | `fix now | retain | defer` | Preserve Honor channel gate regardless of strategy. |
...
- permission and channel boundary preservation, especially Honor ranking gates
...
- [ ] Each legacy path has an explicit approved strategy: redirect/help, compatibility shim,
  retained legacy path, or removal after announcement.
- [ ] `/kvk rankings` remains the canonical player ranking surface.
- [ ] Ranking semantics, SQL contracts, My Rank, CSV export, visual Top 10 cards, compact Top
  25/50 output, and Hall of Fame records semantics are preserved unless explicitly approved.
- [ ] Honor no-admin-override channel behaviour is preserved.
- [ ] No new direct SQL exists in command or view modules.
- [ ] Command registration docs/tests/baselines are updated for any approved command-surface
  change.
- [ ] User/operator docs and smoke references match the delivered command surface.

...
- Active Task C Slice 5 files:
  - `Codex Task Pack - Import Pipeline Deferred Optimisation Task C Slice 5 PreKvK Import Audit Adoption.md`
  - `Codex Chat Starter - Import Pipeline Deferred Optimisation Task C Slice 5 PreKvK Import Audit Adoption.md`
- Task C Slice 5 is the next import architecture cleanup slice. It starts with audit/scope and SQL
  implementation-boundary confirmation for PreKvK-only generic import audit adoption. It preserves
  PreKvK route UX, embed text, file handling, importer transaction behavior, duplicate/rejection
  behavior, import-history semantics, ranking/report refresh, telemetry, SQL outputs, and
  user-facing behavior. Weekly activity, MGE, inventory generic audit adoption, `dbo.UPDATE_ALL2`
  wrapper instrumentation, `dbo.IMPORT_STAGING_PROC` decomposition, `dbo.UPDATE_ALL2`
  decomposition, residual `stats_module.py` cleanup, and legacy PreKvK SQL cleanup remain separate
  later slices unless explicitly approved.
...
Import Pipeline Deferred Optimisation Task C Slice 5 PreKvK Import Audit Adoption is the next
prepared import pipeline slice. It starts with audit/scope and SQL implementation-boundary
confirmation for adopting generic durable audit on the PreKvK upload/import path only. Weekly
activity, MGE, inventory adoption, fallback/player-location/Honor behavior changes, route/command

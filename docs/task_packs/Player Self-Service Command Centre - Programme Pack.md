...
Active Phase 13 files:

- `docs/task_packs/Codex Task Pack - Player Self-Service Command Centre Phase 13 Legacy Redirect Planning.md`
- `docs/task_packs/Codex Chat Starter - Player Self-Service Command Centre Phase 13 Legacy Redirect Planning.md`
...
Phase 13 should audit remaining legacy self-service paths, command usage, player communication
needs, compatibility risks, and redirect/removal options. Do not redirect or remove
`/register_governor`, `/modify_registration`, `/my_registrations`, `/mygovernorid`,
`/subscribe`, `/modify_subscription`, `/unsubscribe`, `/inventory_preferences`,
`/my_stats_export`, or `/export_inventory` without explicit operator approval, player
communication, and a no-feedback monitoring window. Export schema/format redesign remains a
separate export-output programme unless explicitly narrowed later.

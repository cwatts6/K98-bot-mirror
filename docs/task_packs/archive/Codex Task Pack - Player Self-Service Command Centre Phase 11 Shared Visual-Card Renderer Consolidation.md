...
- Status: `archived after Phase 11A, Phase 11B, and Phase 11C were delivered and smoke tested; Phase 11 complete`
...
2026-06-26. It migrated KVK stats and targets directly to `core.visual_text`, kept rankings and
history on KVK-local helpers backed by the shared primitive layer, aligned KVK fit/width
measurement with glyph-safe clustered drawing, added focused ownership and fitting tests, included
`phase11b_kvk_stats_smoke.png` and `phase11b_kvk_rankings_smoke.png`, and preserved KVK visual
output contracts, filenames, dimensions, fallback behavior, special-character player-name
handling, and public/private command behavior.

Phase 11C is delivered in production PR #483 and smoke tested successfully by the operator on
2026-06-26, including special-character rendering. It migrated Inventory report font loading,
glyph-safe text width, fit-to-width sizing, wrapping, and text drawing to `core.visual_text` while
preserving Inventory report layout, dimensions, filenames, report visibility behavior, range
controls, export buttons, fallback behavior, and generated report contracts.
...
2. Phase 11B: delivered and smoke tested. Migrated KVK stats and targets directly to the shared
...
Inventory migration is no longer an active deferred item. Phase 11 is complete unless the operator
explicitly opens a new renderer consolidation phase later.

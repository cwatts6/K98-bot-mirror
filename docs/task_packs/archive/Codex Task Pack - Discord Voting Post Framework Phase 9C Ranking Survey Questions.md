...
- Task type: `audit | product scope | SQL-backed survey extension design | Discord interaction UX | privacy/export review | implementation closeout`
- One-pass approved: `yes, after architecture/product/SQL/UX direction was approved`
- Status: `complete; SQL deployed; bot runtime smoke tested; archived as delivery record`

## 2. Objective

Audit, design, and deliver the next advanced survey slice: ranking survey questions.
...
The audit/scope packet was completed first, then implementation proceeded after approval. Runtime
delivery was kept to complete ranking survey questions only. Dashboard/reporting, draft/resume,
rating-scale extensions, emoji/icon support, public detail export, role/governor voting, saved
templates, and `/vote_admin` reshaping were not included.
...
Phase 1 through Phase 9B were complete and smoke tested before this slice. The voting framework supports SQL-backed
...
## 4A. Phase 9C Delivered Closeout

Phase 9C is complete. It delivered complete ranking survey questions over the existing 2-6 survey
option model, using the required/optional semantics delivered in Phase 9A and preserving Phase 9B
rating behavior.

Delivered through:

- Bot mirror PR: `cwatts6/K98-bot-mirror#204`
- SQL PR: `cwatts6/K98-bot-SQL-Server#33`
- SQL migration: `20260704_003_add_survey_ranking_questions`
- Production smoke test: `2026-07-04`

Delivered scope:

- `QuestionType = Ranking` for SQL-backed surveys.
- Dedicated `dbo.SurveyRankingAnswers` storage with one row per ranked option.
- SQL uniqueness constraints for one rank per option and one option per rank for each
  response/question.
- Runtime guards for bot-before-SQL deployment ordering, with clear migration errors if ranking
  storage is missing.
- Required ranking questions require every option to be ranked once before submit.
- Optional ranking questions can be skipped or cleared; partial ranking remains out of scope.
- Builder controls expose `Ranking` through guided question-type selection without free-typed type
  values.
- Player private response flow supports entering, reviewing, editing, prefilled editing, duplicate
  prevention, skipping optional rankings, clearing optional rankings, and submission.
- PublicLive and HiddenUntilClose ranking output is aggregate-only.
- Public card/summary output shows average rank, first-place count, and rank distribution for
  submitted ranking answers.
- Private admin/leadership status shows ranking aggregate information.
- Private closed-only totals and response-detail exports include ranking aggregate/ranked-option
  rows, skipped optional rankings, original rank values, and changed flags where applicable.
- Formula-safe CSV cells and spreadsheet-safe Discord IDs were preserved.
- Audit JSON records counts/status metadata rather than full ranking answer payloads.
- Restart-safe public survey opener behavior was preserved.
- Existing one-choice votes, multi-select votes, choice/text/detail/optional surveys, and fixed
  1-5 rating surveys remain compatible.

Review hardening:

- Missing ranking/rating storage guards now preserve clear `VoteValidationError` messages even
  when `exec_with_cursor` swallows callback exceptions.
- Cleared optional rankings now retain original per-option ranks in response-detail audit rows so
  `RankingChanged` and `ResponseChanged` remain correct.

Validation evidence:

- Focused survey DAL/export regression tests: `23 passed`.
- Full bot test suite after review fixes: `2311 passed, 2 skipped`.
...
- SQL repo validation passed for the ranking migration with only historical warnings.
...
- Public card output displays ranking aggregate summaries.
- Existing vote/survey behavior remains compatible.

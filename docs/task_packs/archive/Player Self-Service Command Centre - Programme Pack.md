...
Status as of 2026-06-27 after Phase 13 production smoke:
...
- Phase 11C Inventory Renderer Migration is delivered in production PR #483 and smoke tested
  successfully by the operator on 2026-06-26. It migrates `inventory/report_image_renderer.py`
  font loading, glyph-safe text width, fit-to-width, wrapping, and drawing paths to
  `core.visual_text`, while keeping Inventory-local chart, panel, PNG export, filename, report
  visibility, range-control, and export-button behavior unchanged. Focused tests cover shared
  helper ownership and special-character governor-name rendering, the local
...
- Phase 12B Discord User Preference Profile Store is delivered in mirror PR #177, SQL PR #20,
  ...
  readable names in the bot service, uses guided dropdowns instead of free-text entry, replaces
  the private Manage Profile child window after each update, and preserves the current
  session-based local-time toggle.
...
- Phase 13 Legacy Redirects, Briefing, and Cleanup is delivered in production PR #486 and smoke
  tested successfully by the operator on 2026-06-27. Approved account, reminder, calendar
  reminder, inventory preference, and export legacy commands remain registered but now return
  private redirect guidance to the matching `/me` centre. Slash-command picker descriptions now
  advertise the deprecated redirect behavior. The operator-provided SQL extract and dated JSONL
  files showed nonzero broad usage for every audited legacy and related personal path, so final
  command-registration removal remains a separate deferred cleanup after player communication,
  post-redirect usage monitoring, a no-feedback window, and operator approval. Export schema
  redesign remains separate.
- The Player Self-Service Command Centre programme is complete. All planned `/me` surfaces are
  delivered and smoke tested, and the approved legacy redirect rollout is complete. Remaining
  stats/profile/detailed-inventory alignment is outside this first programme and is seeded as the
  Player Self-Service Command Centre v2 audit/design programme.
...
- Broader preferences hub expansion is now promoted to Phase 12 after launch and renderer
  consolidation decisions were completed.
...
- Shared visual-card renderer helper consolidation was later delivered in Phase 11.
...
- `/my_stats_export` and `/export_inventory` remained registered and usable at Phase 9; Phase 13
  later redirected both privately to `/me exports` after explicit operator approval.
...
Status: Phase 11A, Phase 11B, and Phase 11C delivered and smoke tested; Phase 11 complete.
...
- aligned KVK text fitting and width checks with `core.visual_text.text_width`, including
  bold-aware fit paths, so glyph-safe clustered drawing and measurement use the same primitive
  path for long, emoji, and CJK-like player names
...
- smoke tested `/kvk stats`, `/kvk targets`, `/kvk rankings`, and `/kvk history` successfully on
  2026-06-26, including special-character governor names; `phase11b_kvk_rankings_smoke.png`
  captures the operator-verified Top 10 rankings case
...
- smoke tested successfully by the operator on 2026-06-26, including special-character rendering
...
- Phase 11C delivered the Inventory report renderer migration to the shared helper while preserving
  report layout, filenames, dimensions, and existing report/export behavior. Operator smoke
  completed successfully on 2026-06-26, so Phase 11 is complete unless the operator requests a
  new renderer consolidation phase later.
...
Archived Phase 12 files:

- `docs/task_packs/archive/Codex Task Pack - Player Self-Service Command Centre Phase 12 Preferences Hub Expansion.md`
- `docs/task_packs/archive/Codex Chat Starter - Player Self-Service Command Centre Phase 12 Preferences Hub Expansion.md`

### Phase 12B — Discord User Preference Profile Store

Status: delivered in mirror PR #177, SQL PR #20, and production PR #485; smoke tested
...
- `/me preferences` card display and Manage Profile add/update/remove controls for delivered
  fields
- guided dropdown controls for timezone, location country, and preferred language so players do
  not need to know IANA timezone names, country codes, or language tags
- in-place replacement of the private Manage Profile child window after save/clear actions so
...
Phase 12B manual smoke evidence:
...
- Timezone, location country, and preferred language saved, updated, cleared, and reloaded across
  fresh `/me preferences` interactions.
- Guided dropdowns worked for all three profile fields and improved the player UX over free-text
  entry.
...
- The private dropdown child window was replaced correctly after updates; no additional child
  windows were created after repeated changes.
- Existing Inventory visibility and Inventory VIP flows remained behavior-compatible.

Archived Phase 12B files:

- `docs/task_packs/archive/Codex Task Pack - Player Self-Service Command Centre Phase 12B Discord User Preference Profile Store.md`
- `docs/task_packs/archive/Codex Chat Starter - Player Self-Service Command Centre Phase 12B Discord User Preference Profile Store.md`

### Phase 13 — Legacy Redirects, Briefing, and Cleanup

Status: delivered in production PR #486 and smoke tested successfully by the operator on
2026-06-27. Phase 13 is complete and archived.
...
- Supplied SQL extract and dated JSONL evidence show direct legacy usage still exists and `/me`
  usage was concentrated in the smoke-test/operator window before rollout, so final
  command-registration removal still requires player communication, monitoring, usage review, and
  operator approval.
- Operator smoke on 2026-06-27 confirmed all approved redirects are correct.
...
Archived Phase 13 files:

- `docs/task_packs/archive/Codex Task Pack - Player Self-Service Command Centre Phase 13 Legacy Redirect Planning.md`
- `docs/task_packs/archive/Codex Chat Starter - Player Self-Service Command Centre Phase 13 Legacy Redirect Planning.md`

Programme archive record:
...
- Delivered shared visual-card renderer/helper consolidation for the `/me` visual-card model and
  adjacent KVK, PreKvK, and inventory renderers after the player self-service card surfaces became
  stable.
...
- Legacy redirects, player briefing, and smoke-validated redirect rollout.
...
- Final command-registration removal for redirect-only legacy paths after player communication,
  no-feedback monitoring, production usage review, and operator approval.
- Player Self-Service v2 audit/design for `/my_stats`, `/stats player`, `/player_profile`, and
  `/myinventory`; `/mykvkcrystaltech` remains unique and should only be reviewed for product fit.
...
## 18. Programme Closure And Next Action

This programme is complete after Phase 13. The delivered command centre now covers dashboard,
accounts, reminders, preferences, inventory summary, exports, profile preferences, generated card
surfaces, and approved legacy redirects.

Remaining work is outside this programme:

- no-feedback monitoring and possible final removal of redirect-only legacy registrations
- Player Self-Service Command Centre v2 audit/design for personal stats, leadership/profile, and
  detailed inventory report alignment
- export schema/format redesign as a separate export-output programme
- public calendar/KVK calendar redesign as a separate calendar UX programme

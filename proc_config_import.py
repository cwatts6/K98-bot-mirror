...
# NEW: import log health helper
from log_health import LogHeadroomError, log_backup_context, preflight_or_raise

# Honor environment overrides (you added these to .env)
...
def _is_transient_error(exc: BaseException) -> bool:
    # Delegated to central logic in gsheet_module; keep simple fallback here.
...
def _get_sheet_service():
    """
    Build a Sheets service (readonly) using the shared robust transport helper in gsheet_module.
    Delegates to gsheet_module._build_sheets_with_timeout (which we imported).
    """
    scopes = ["https://www.googleapis.com/auth/spreadsheets.readonly"]
    creds = service_account.Credentials.from_service_account_file(CREDENTIALS_FILE, scopes=scopes)
    # Try extended signature first (some versions accept extra kwargs)
    try:
...
# --- NEW helper: safe quote SQL server identifiers (very small/local helper) ---
def _quote_sql_columns(cols) -> str:
    """
    ...
    returns: comma-separated quoted identifiers like [Col Name],[Other]
    """
    quoted = []
    for c in cols:
        s = str(c)
...
def run_proc_config_import() -> bool:
    ...
    try:
        ...
        # NEW: preflight check immediately after connecting; abort cleanly if insufficient headroom
        try:
            preflight_or_raise(conn, dbname=DATABASE, warn_threshold=85.0)
        except LogHeadroomError as e:
            print(f"[ABORT] SQL log headroom insufficient: {e}")
            try:
                cursor.close()
            except Exception:
                pass
        ...
        # quote columns safely for SQL Server
        cols_sql = _quote_sql_columns(df.columns)
        insert_sql = f"INSERT INTO dbo.ProcConfig_Staging ({cols_sql}) VALUES ({','.join(['?'] * len(df.columns))})"
        ...
        try:
            ...
            if not df_bands.empty:
                ...
                cols_sql = _quote_sql_columns(df_bands.columns)
                insert_sql = f"INSERT INTO dbo.KVKTargetBands ({cols_sql}) VALUES ({','.join(['?'] * len(df_bands.columns))})"
                fe = _enable_fast_executemany(cursor)
        ...
        try:
            ...
            if not df_exempt.empty:
                ...
                cols_sql = _quote_sql_columns(df_exempt.columns)
                insert_sql = f"INSERT INTO dbo.EXEMPT_FROM_STATS ({cols_sql}) VALUES ({','.join(['?'] * len(df_exempt.columns))})"
                fe = _enable_fast_executemany(cursor)

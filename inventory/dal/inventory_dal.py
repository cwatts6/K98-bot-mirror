    ...
    try:
        ...
        cur.execute(
            ...
            WHERE GovernorID = ?
              AND ImportType = ?
              AND Status = 'approved'
              AND CAST(ApprovedAtUtc AS date) = CAST(SYSUTCDATETIME() AS date)
            """,
    ...
    *,
    ...
    try:
        ...
        cur.execute(
            ...
            UPDATE dbo.InventoryImportBatch
            SET Status = ?,
                ApprovedAtUtc = COALESCE(?, ApprovedAtUtc),
                RejectedAtUtc = COALESCE(?, RejectedAtUtc),
                CorrectedJson = COALESCE(?, CorrectedJson),
                FinalJson = COALESCE(?, FinalJson),
                ErrorJson = COALESCE(?, ErrorJson)
            WHERE ImportBatchID = ?
            """,
            (
                str(status.value),

...
async def approve_import(
    *,
    ...
    if summary.import_type == InventoryImportType.RESOURCES:
        await asyncio.to_thread(
            inventory_dal.insert_resource_records,
            import_batch_id=int(import_batch_id),
            governor_id=int(governor_id),
            scan_utc=scan_utc,
            resources=normalized["resources"],
        )
    elif summary.import_type == InventoryImportType.SPEEDUPS:
        await asyncio.to_thread(
            inventory_dal.insert_speedup_records,
            import_batch_id=int(import_batch_id),
            governor_id=int(governor_id),
            scan_utc=scan_utc,
    ...
    await asyncio.to_thread(
        ...
        import_batch_id=int(import_batch_id),
        status=InventoryImportStatus.APPROVED,

...
Enhancements added:
- module-level usage_event(name, value=1, metadata=None) for recording simple metrics
  (synchronous API safe to call from non-async code).
- metrics stored to daily JSONL (metrics_YYYYMMDD.jsonl) and enqueued for SQL via AsyncUsageTracker.
- in-memory sliding-window counters for each metric name to detect spikes.
- configurable alert thresholds (env):
    USAGE_ALERT_THRESHOLD (default 10)
    USAGE_ALERT_WINDOW (seconds, default 60)
    USAGE_ALERT_SUPPRESS (seconds suppress repeat alerts, default 300)
- when threshold exceeded, emit an alert: writes alerts JSONL and logs a WARNING; also attempts
  to enqueue an alert event via the AsyncUsageTracker.
- small helper functions to inspect and flush metrics.
"""
...
def _ensure_global_tracker() -> AsyncUsageTracker:
    ...
    with _GLOBAL_LOCK:
        if _GLOBAL_TRACKER is None:
            ...
            # Attempt to start if an event loop is running
            try:
                loop = asyncio.get_running_loop()
                _GLOBAL_TRACKER.start()
            except RuntimeError:
                # No running loop; the caller or application should start the tracker when ready

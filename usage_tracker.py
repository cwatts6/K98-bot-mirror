        ...
        try:
            ...
            try:
                if run_blocking_in_thread is not None:
                    # schedule background worker with sensible telemetry/meta
                    meta = {
                        "command_name": evt_copy.get("command_name"),
                        "app_context": evt_copy.get("app_context"),
                    }
                    asyncio.create_task(
                        run_blocking_in_thread(
                            _sync_write,
                            evt_copy,
                            name="usage_local_jsonl_write",
                            meta=meta,
                        )
                    )
                else:
                    ...
                    log.debug(
                        "[USAGE] run_blocking_in_thread unavailable; using asyncio.to_thread fallback for local JSONL write (consider converting to run_blocking_in_thread)"
                    )

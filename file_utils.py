...
def _is_winerr32(e: BaseException) -> bool:
    """
    Detect Windows sharing violation / file-in-use errors robustly.
    """
    try:
        # Windows native code often available as .winerror
        if getattr(e, "winerror", None) == 32:
            return True
    except Exception:
        pass
    try:
        # Sometimes surfaced as PermissionError with errno EACCES
        if isinstance(e, PermissionError) and getattr(e, "errno", None) in (errno.EACCES, 13):
            msg = str(e).lower()
            if "being used by another process" in msg or "cannot access the file" in msg:
                return True
    except Exception:
        pass
    return False
...
def atomic_write_json(
    ...
    *,
    ...
    replace_backoff_base: float = 0.02,  # 20ms
    replace_backoff_max: float = 0.25,  # 250ms
) -> None:
    p = Path(path)
    if ensure_parent_dir:
        _ensure_parent(p)

    # keep deterministic tmp name behavior (minimal change footprint)
    ...
    # retry os.replace for transient WinError 32 collisions
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            if _is_winerr32(e) and attempt < attempts:
                # exponential backoff + full jitter
                exp = min(replace_backoff_base * (2 ** (attempt - 1)), replace_backoff_max)
                sleep_s = random.uniform(0.0, exp)
                try:
                    logger.warning(
                        "[atomic_write_json] WinError32 contention on %s (attempt %d/%d), retrying in %.3fs",
                        p,
                        attempt,
                        attempts,
                        sleep_s,
            ...
            try:
                # cleanup best-effort so stale .tmp doesn't accumulate on repeated failures
                if tmp.exists():
    ...
    *,
    ...
    try:
        ...
        for attempt in range(1, attempts + 1):
            try:
                os.replace(tmp, path)
                consumed = True  # mark as consumed so finally block skips removal
                break
            except Exception as exc:
                # ✅ reuse _is_winerr32() for robust Windows sharing-violation detection
                if _is_winerr32(exc) and attempt < attempts:
                    # ✅ full-jitter exponential backoff matching atomic_write_json
                    exp = min(replace_backoff_base * (2 ** (attempt - 1)), replace_backoff_max)
                    sleep_s = random.uniform(0.0, exp)
                    try:
                        logger.warning(
                            "[atomic_json_write] WinError32 contention on %s (attempt %d/%d), retrying in %.3fs",
                            path,
                            attempt,
                            attempts,
                            sleep_s,

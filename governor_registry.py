...
def register_account(
    ...
    # Prevent duplicate GovernorID claimed by another user
    if _is_claimed_by_other(data, governor_id, discord_id):
...
class KVKAccountButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Build and send
        try:
            embed, file = build_stats_embed(gov_data, interaction.user)
...
class ConfirmRemoveView(discord.ui.View):
    ...
    async def cancel(self, button, interaction: discord.Interaction):
        await interaction.response.send_message("✎ Account removal cancelled.", ephemeral=True)
        self.stop()

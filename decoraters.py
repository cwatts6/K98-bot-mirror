...
try:
    ...
    raise ValueError("ADMIN_USER_ID must be set and coercible to int") from exc

# Be robust when optional values are missing: only include configured IDs
ALLOWED_CHANNEL_IDS = {
    int(x)
    for x in (NOTIFY_CHANNEL_ID, LEADERSHIP_CHANNEL_ID, ARK_SETUP_CHANNEL_ID)
    if x is not None
}
...
async def _safe_ephemeral_send(interaction: discord.Interaction, message: str):
    ...
    try:
        ...
        # Log at debug to help trace potential issues without spamming logs.
        logger.debug("ephemeral send failed (suppressed)", exc_info=False)
...
def _is_admin(user: discord.abc.User | None) -> bool:
    try:
        ...
        # If user object lacks id or conversion fails, treat as not admin
        return False
...
def is_admin_and_notify_channel(allow_leadership: bool = False):
    ...
    def decorator(func):
        ...
        async def wrapper(arg0, *args, **kwargs):
            ...
            # Build allow list; prefer the pre-computed ALLOWED_CHANNEL_IDS but
            # keep explicit NOTIFY channel first for behavior parity.
            ...
            async def _log_denied(reason: str):
                try:
                    ...
                    logger.debug(
                        "usage_tracker.log failed in _log_denied (suppressed)", exc_info=False
                    )
...
def is_admin_or_leadership_only():
    """
    Gate: (Admin by ADMIN_USER_ID) OR (has Leadership role).
    No channel restriction (compose with @channel_only when needed).
    """
...
def channel_only(
    ...
    *,
    ...
    - channels: int or iterable of ints/strings coercible to int
    ...
    def decorator(func):
        ...
        async def wrapper(arg0, *args, **kwargs):
            ...
            # allowed if channel id matches or (threads allowed and parent matches)
            if (chan_id in allowed) or (allow_threads and parent_id in allowed):
                return await func(arg0, *args, **kwargs)

            # build mention text and inform the user privately (ephemeral)
            mentions = " or ".join(f"<#{cid}>" for cid in sorted(allowed))
            # supply helpful hint only to the invoking user
            await _safe_ephemeral_send(
                inter, f"❌ This command may only be used in {mentions}. Please try there."
            )
            return
...
def _safe_args_preview(kwargs: dict[str, Any] | None, *, max_len: int = 32):
    ...
    for k, v in kwargs.items():
        kn = str(k).lower()
        # Redact if any sensitive fragment appears in the key name
        if any(sk in kn for sk in SENSITIVE_KEYS):
            out[k] = "***"
            continue
...
def track_usage(command_name: str | None = None, app_context: str = "slash"):
    ...
    def _unwrap(obj, depth=3):
        # Walk wrapper chain a few levels and return the unwrapped object.
        ...
        async def wrapper(ctx, *args, **kwargs):
            ...
            try:
                ...
                try:
                    ...
                    logger.warning("[USAGE] queue log failed", exc_info=False)

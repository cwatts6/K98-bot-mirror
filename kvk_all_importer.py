...
def _coerce(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def as_dt(x):
        ...
        try:
            ...
            return pd.to_datetime(x, errors="coerce").to_pydatetime().replace(tzinfo=None)
        except Exception:
            return None
...
def ingest_kvk_all_excel(
    *,
    ...
    try:
        ...
        # Discord timestamps are aware (UTC). Our proc expects naive UTC; strip tzinfo.
        if getattr(scan_ts_utc, "tzinfo", None) is not None:
            scan_ts_utc = scan_ts_utc.astimezone(dt.UTC).replace(tzinfo=None)

        # Commit ingest (TIMED)
        cur = con.cursor()
        t_ingest0 = time.perf_counter()
        cur.execute(
            CALL_INGEST_SQL, (token, scan_ts_utc, source_filename, file_hash, int(uploader_id))
        )
        res = cur.fetchall()

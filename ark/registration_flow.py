...
from ark.dal.ark_dal import (
    ...
    get_signup,
    insert_audit_log,
    mark_registration_announcement,
    move_signup_slot,
    reactivate_signup,
    remove_signup,
    switch_signup_governor,
    touch_registration_refresh,
...
def _resolve_admin_add_discord_user_id(
    *,
    ...
    existing_discord_user_id: int | None = None,
) -> int | None:
    """Resolve Discord identity for admin-added Ark rows without overriding existing identity."""
    if existing_discord_user_id:
        logger.info(
            "ark_admin_add_discord_identity_preserved governor_id=%s resolved_discord_id=%s actor_discord_id=%s",
            governor_id,
            existing_discord_user_id,
            actor_discord_id,
        )
        return int(existing_discord_user_id)

    owner = get_discord_user_for_governor(governor_id)
    if not owner:
        logger.info(
            "ark_admin_add_discord_identity_not_found governor_id=%s actor_discord_id=%s",
            governor_id,
            actor_discord_id,
        )
        return None

    resolved_discord_id = int(owner["DiscordUserID"])
    logger.info(
        "ark_admin_add_discord_identity_resolved governor_id=%s resolved_discord_id=%s actor_discord_id=%s",
        governor_id,
        resolved_discord_id,
        actor_discord_id,
    )
...
def _read_json_registration_ref(
    state: ArkJsonState, match_id: int
) -> tuple[ArkMessageRef | None, bool]:
    """Read legacy JSON registration state, supporting current and older shapes."""
    msg_state = state.messages.get(int(match_id))
    if msg_state and msg_state.registration:
        return msg_state.registration, True
    return None, False
...
class ArkRegistrationController:
    ...
    async def _safe_send(
        ...
        *,
        ...
        if self._response_is_done(interaction):
            await interaction.followup.send(message, **payload)
        else:
            await interaction.response.send_message(message, **payload)
    ...
    async def refresh_registration_message(self, client) -> None:
        try:
            ...
            logger.exception(
                "[ARK] Failed to refresh registration message for match_id=%s", self.match_id
            )
    ...
    async def _persist_registration_state(
        ...
        *,
        match_id: int,
        registration_ref: ArkMessageRef,
        announce_sent: bool,
        touched_refresh_at: datetime | None,
    ) -> None:
        await update_match_registration_message(
            match_id,
            int(registration_ref.channel_id),
            int(registration_ref.message_id),
        )
        if announce_sent:
            await mark_registration_announcement(
                match_id,
                announcement_sent=True,
                announcement_sent_at_utc=datetime.now(UTC),
            )
        if touched_refresh_at is not None:
            await touch_registration_refresh(match_id, touched_refresh_at)

    async def _save_registration_tracker(
        self,
        *,
        match_id: int,
        channel_id: int,
        ...
        await save_view_tracker_async(
            f"arkmatch_{match_id}",
            {
                ...
                "events": [
                    serialize_event(
                        {
                            "name": f"Ark Match — {match.get('Alliance')}",
                            "start_time": match_dt,
                        }
                    )
                ],
    ...
    async def ensure_registration_message(
        self,
        *,
        client,
        announce: bool = False,
        ...
        update_refresh_timestamp: bool = False,
    ) -> ArkMessageRef | None:
        ...
        match_row_registration_channel_id = match.get("RegistrationChannelId")
        match_row_registration_message_id = match.get("RegistrationMessageId")
        announce_already_sent = bool(match.get("AnnouncementSent"))

        resolved_target_channel_id = target_channel_id or match_row_registration_channel_id
        ...
        current_json_ref, had_json_ref = _read_json_registration_ref(state, self.match_id)
        effective_current_ref: ArkMessageRef | None = None
        if match_row_registration_channel_id and match_row_registration_message_id:
            effective_current_ref = ArkMessageRef(
                channel_id=int(match_row_registration_channel_id),
                message_id=int(match_row_registration_message_id),
            )
        elif current_json_ref:
            effective_current_ref = current_json_ref
            await self._persist_registration_state(
                match_id=self.match_id,
                registration_ref=current_json_ref,
                announce_sent=announce_already_sent,
                touched_refresh_at=None,
            )
            logger.info(
                "[ARK] Migrated registration refs from JSON to SQL match_id=%s", self.match_id
            )

        existing_state = state.messages.get(self.match_id) or ArkMessageState()
        existing_state.registration = effective_current_ref
        state.messages[self.match_id] = existing_state
        should_announce = bool(announce and (force_announce or not announce_already_sent))
        logger.info(
            "[ARK_REGISTRATION] ensure_message match_id=%s announce_requested=%s force_announce=%s announcement_already_sent=%s should_announce=%s force_repost=%s has_existing_ref=%s",
            self.match_id,
            announce,
            force_announce,
        ...
        delivered, state_changed = await upsert_registration_message(
            ...
            embed=embed,
            view=view,
            target_channel_id=int(resolved_target_channel_id),
            delete_old=force_repost,
    ...
    async def _check_active_ban(
        ...
        *,
        ...
        return await get_active_ban_for(
    ...
    async def _apply_join(
        ...
        *,
        ...
        ban = await self._check_active_ban(
            discord_user_id=interaction.user.id,
            governor_id=int(governor_id),
            ark_weekend_date=match["ArkWeekendDate"],
        )
        if ban:
            await _send_ephemeral(
            ...
            logger.info(
                "ark_signup_blocked_by_ban",
                extra={
                    "match_id": self.match_id,
                    "alliance": match.get("Alliance"),
                    "ark_weekend_date": str(match.get("ArkWeekendDate")),
                    "discord_user_id": interaction.user.id,
                    "governor_id": int(governor_id),
    ...
    async def _prompt_admin_slot_selection(
        ...
        *,
        ...
        await self._safe_send(interaction, msg, view=view, ephemeral=True)
    ...
    async def _apply_admin_add(
        ...
        *,
        ...
        if slot_type == "Player" and len(players) >= int(self.config["PlayersCap"]):
            await self._safe_send(interaction, "❌ Player slots are full.", ephemeral=True)
            return

        if slot_type == "Sub":
            if len(subs) >= int(self.config["SubsCap"]):
                await self._safe_send(interaction, "❌ Sub slots are full.", ephemeral=True)
                return
            if (
        ...
        if any(str(r.get("GovernorId")) == str(governor_id) for r in roster):
            await self._safe_send(
                interaction,
                "❌ That governor is already signed up for this match.",
                ephemeral=True,
            )
            return
        ...
        if ban and not admin_override_ban_rule(self.config):
            await self._safe_send(
                interaction,
                format_ban_block_message(
                    admin_context=True,
        ...
        if ban and admin_override_ban_rule(self.config):
            await interaction.followup.send(
        ...
        existing = await get_signup(self.match_id, int(governor_id))
        resolved_discord_user_id = await asyncio.to_thread(
            _resolve_admin_add_discord_user_id,
            governor_id=int(governor_id),
            actor_discord_id=int(interaction.user.id),
            existing_discord_user_id=(
                int(existing["DiscordUserId"])
                if existing and existing.get("DiscordUserId")
                else None
            ),
        )
        if existing and (existing.get("Status") or "").lower() != "active":
            ok = await reactivate_signup(
                ...
                governor_id=int(governor_id),
                governor_name=governor_name,
                discord_user_id=resolved_discord_user_id,
                slot_type=slot_type,
                source="Admin",
            )
        else:
        ...
        if not ok:
            await self._safe_send(interaction, "❌ Failed to add signup.", ephemeral=True)
            return

...
async def mark_no_show(
    ...
    sql = """
        ...
        OUTPUT INSERTED.SignupId
        WHERE MatchId = ? AND GovernorId = ? AND Status = 'Active'
          AND (NoShow = 0 OR NoShow IS NULL);
    """
...
async def add_ban(
    ...
    if not discord_user_id and not governor_id:
        raise ValueError("At least one target is required: discord_user_id or governor_id.")
    if int(banned_ark_weekends) < 1:
        raise ValueError("banned_ark_weekends must be >= 1.")

    sql = """
        INSERT INTO dbo.ArkBans
            (DiscordUserId, GovernorId, Reason, BannedArkWeekends, StartArkWeekendDate, CreatedByDiscordId)
    ...
    row = await run_one_async(
        ...
        (
            ...
            governor_id,
            reason,
            int(banned_ark_weekends),
            start_ark_weekend_date,
            actor_discord_id,
        ),
    )
...
async def list_bans(active_only: bool = True) -> list[dict[str, Any]]:
    sql = """
        SELECT
            ...
            RevokedByDiscordId
        FROM dbo.ArkBans
        WHERE (? = 0 OR RevokedAtUtc IS NULL)
        ORDER BY BanId DESC;
    """
    return await run_query_async(sql, (1 if active_only else 0,))


# Return the most recent non-revoked ban that is active for the given ark_weekend_date,
# where "active" means ark_weekend_date is between StartArkWeekendDate and EndArkWeekendDate (inclusive).
async def get_active_ban_for(
    ...
    rows = await run_query_async(
        sql,
        (
            ark_weekend_date,  # Start <= target
            ark_weekend_date,  # End >= target
            discord_user_id,
            discord_user_id,
            governor_id,
            governor_id,
...
async def list_completed_matches_pending_completion(days_back: int = 7) -> list[dict[str, Any]]:
    sql = """
        ...
        WHERE Status = 'Completed'
          AND CompletionEmbedPostedAtUtc IS NULL
          AND ArkWeekendDate >= DATEADD(day, -?, CAST(GETUTCDATE() AS date))
        ORDER BY ArkWeekendDate DESC, MatchTimeUtc DESC;
    """
    return await run_query_async(sql, (int(days_back),))
...
async def mark_match_completion_posted(match_id: int) -> bool:
    sql = """
        ...
        SET CompletionEmbedPostedAtUtc = SYSUTCDATETIME(),
            UpdatedAtUtc = SYSUTCDATETIME()
        OUTPUT INSERTED.MatchId
        WHERE MatchId = ? AND CompletionEmbedPostedAtUtc IS NULL;
    """
    row = await run_one_async(sql, (match_id,))
    return int((row or {}).get("MatchId") or 0) > 0
...
async def upsert_team_preference(
    ...
    sql = """
        ...
        UPDATE dbo.ArkTeamPreferences WITH (HOLDLOCK)
        SET PreferredTeam = @PreferredTeam,
            IsActive = 1,
            UpdatedAtUTC = SYSUTCDATETIME(),
            UpdatedBy = @UpdatedBy
        ...
        INTO @Output (GovernorID, PreferredTeam, IsActive, CreatedAtUTC, UpdatedAtUTC, UpdatedBy)
        WHERE GovernorID = @GovernorID;

        IF @@ROWCOUNT = 0
        BEGIN
            INSERT INTO dbo.ArkTeamPreferences (GovernorID, PreferredTeam, IsActive, CreatedAtUTC, UpdatedAtUTC, UpdatedBy)
            OUTPUT inserted.GovernorID, inserted.PreferredTeam, inserted.IsActive,
                   inserted.CreatedAtUTC, inserted.UpdatedAtUTC, inserted.UpdatedBy
            INTO @Output (GovernorID, PreferredTeam, IsActive, CreatedAtUTC, UpdatedAtUTC, UpdatedBy)
            VALUES (@GovernorID, @PreferredTeam, 1, SYSUTCDATETIME(), SYSUTCDATETIME(), @UpdatedBy);
        END;

        SELECT GovernorID, PreferredTeam, IsActive, CreatedAtUTC, UpdatedAtUTC, UpdatedBy
        FROM @Output;
    """
    return await run_one_async(sql, (governor_id, preferred_team, updated_by))

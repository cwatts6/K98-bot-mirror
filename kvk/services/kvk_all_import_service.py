...
def _numeric_series(df: pd.DataFrame, column: str) -> pd.Series:
    ...
    integer_literal_mask = pd.Series(False, index=df.index, dtype=bool)
    if string_mask.any():
        string_values = source.loc[string_mask]
        integer_literal_mask.loc[string_mask] = (
            string_values.astype(str).str.strip().str.fullmatch(r"[+-]?\d+").fillna(False)
        )
    invalid_string_mask = string_mask & source.notna() & ~integer_literal_mask

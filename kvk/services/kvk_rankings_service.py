...
def _clean_rank_message_text(value: Any) -> str:
...
async def build_my_rank_lookup_result(
    *,
    ...
    if row_index is None:
        safe_governor_name = _clean_rank_message_text(governor_name)
        return _my_rank_result(
            status="not_ranked",
            mode=payload.mode,
            metric=payload.metric,
            ...
            metric_label=payload.metric_label,
            message=(
                f"{safe_governor_name} (`{selected_governor_id}`) is not ranked for "

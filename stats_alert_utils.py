...
# Import both fetch_all_dicts and cursor_row_to_dict from stats_helpers
from stats_helpers import cursor_row_to_dict, fetch_all_dicts

# timezone helpers (date_to_utc_start already added to utils)
...
def _claim_send(kind: str, *, max_per_day: int = 1) -> bool:
    ...
    try:
        with acquire_lock(lock_path, timeout=5):
            ...
            # Use consistent UTC time formatting
...
async def _send_prekvk_embed(
    ...
    *,
    ...
    # Date range (best-effort) — use the shared formatter
    try:
        kvk_date_range = (

...
import asyncio
import csv
from datetime import UTC, datetime, timedelta  # ensure timedelta is imported
import json
import logging
import os
import time
...
LOG_PATH = os.path.abspath(STATS_ALERT_LOG)
...
def _ensure_log_exists() -> None:
    ...
    if not os.path.exists(LOG_PATH):
        ...
        log.info("[STATS_ALERT] created log at %s", LOG_PATH)
    else:
        _migrate_log_if_needed()  # <- NEW: ensure header & 3 columns


def _migrate_log_if_needed() -> None:
    """
    One-time upgrader:
    - If file lacks header, add it.
    - If any row has only 2 fields (date,time), append default kind='offseason_daily'.
    Writes atomically.
    """
    try:
        ...
        # Detect header
        first = lines[0]
        has_header = first.lower().startswith("date,") and "time" in first.lower()
        ...
        for ln in rows:
            ...
            if len(parts) == 2:
                # Legacy row -> assume offseason_daily (that’s what we historically logged)
                date_str, time_str = parts
                upgraded.append([date_str, time_str, KIND_OFFSEASON_DAILY])
            elif len(parts) >= 3:
                # Already has kind (keep the first 3 cols)
                upgraded.append(parts[:3])
            else:
                # Corrupt line -> skip
                continue

        # If nothing changed but header missing, still add it
        out_lines_changed = (not has_header) or any(
            len([p for p in ln.split(",")]) != 3 for ln in rows
        )

        if out_lines_changed:
            tmp = f"{LOG_PATH}.tmp"
            ...
            log.info("[STATS_ALERT] migrated log to headered 3-col format at %s", LOG_PATH)
    except Exception:
        log.exception("[STATS_ALERT] failed to migrate stats alert log")
...
def _to_aware_utc(dt):
    ...
    if dt.tzinfo is None or dt.tzinfo.utcoffset(dt) is None:
        # Treat naive as UTC (our event sources are UTC)
...
def _iter_log_rows():
    ...
    with open(LOG_PATH, newline="", encoding="utf-8") as f:
        ...
        if first and isinstance(first, list) and first and first[0].lower() == "date":
            ...
            # first was a data row; process it
            if first:
                yield _normalize_row(first)
...
def _save_state(d: dict) -> None:
    try:
        os.makedirs(os.path.dirname(STATE_PATH) or ".", exist_ok=True)  # <-- ensure dir exists
        tmp = f"{STATE_PATH}.tmp"
        with open(tmp, "w", encoding="utf-8") as f:
            json.dump(d, f)
        os.replace(tmp, STATE_PATH)
    except Exception:
        log.exception("[STATS_ALERT] Failed to persist state")
...
def _load_prekvk_top3(kvk_no: int, sql_conn_str: str) -> dict[str, list[dict]]:
    ...
    try:
        ...
        log.exception("[PREKVK] Failed to load Pre-KVK Top-3")
    return out
...
def get_latest_honor_top(n: int = 3) -> list[dict]:
    ...
    try:
        ...
        log.exception("[HONOUR] Failed loading latest Top-%s", n)
        return []
...
def purge_latest_honor_scan() -> int:
    ...
    try:
        with _sql_conn() as cn:
            ...
            cn.commit()
            log.info("[HONOUR] Purged latest scan KVK=%s ScanID=%s (rows=%s)", kvk, sid, cnt)
            return cnt
    except Exception:
        log.exception("[HONOUR] Purge latest scan failed")
        return 0
...
class _CsvSendLock:
    """Simple cross-process lock via O_EXCL file create/delete."""
    ...
    def __enter__(self):
        # Retry briefly in case another process holds the lock
        for _ in range(50):  # ~5s max (50 * 0.1s)
            try:
...
def _claim_send(kind: str, *, max_per_day: int = 1) -> bool:
    ...
    with _CsvSendLock(_lock_path()):
        ...
        with open(LOG_PATH, "a", newline="", encoding="utf-8") as f:
            csv.writer(f).writerow([today, time_str, kind])
            f.flush()
            os.fsync(f.fileno())
        log.info(
...
def _last_send_dt_utc(kind: str) -> datetime | None:
    ...
    for row in _iter_log_rows():
        if not row or row["kind"] != kind:
            continue
        try:
            dt = datetime.strptime(f"{row['date']} {row['time_utc']}", "%Y-%m-%d %H:%M:%S").replace(
                tzinfo=UTC
            )
...
def get_latest_kvk_metadata_sql() -> dict[str, Any] | None:
    ...
    try:
        ...
        log.exception("[KVK SQL META] Failed to read KVK_Details")
        return None
...
def get_latest_kvk_metadata() -> dict[str, Any] | None:
    ...
    try:
        ...
        log.exception("[KVK META] Failed to load KVK metadata")
        return None
    ...
    try:
        ...
        with conn:
            ...
            if not row or row.CurrentKVK is None:
                log.warning("[KVK SQL] No CURRENTKVK3 found in ProcConfig.")
                return None
        ...
        log.exception("[KVK SQL] Failed to read KVK window")
        return None
...
def is_currently_kvk(
    ...
    try:
        ...
        if max_scan_order is None:
            log.warning("[KVK CHECK] No ScanOrder found — treating as off-season.")
            return False

        return match_scan <= max_scan_order <= end_scan

    except Exception:
        log.exception("[KVK CHECK] Unexpected failure in KVK detection logic")
        return False
...
def is_kvk_fighting_open() -> bool:
    ...
    try:
        ...
        if max_scan_order is None:
            log.warning("[KVK CHECK] No ScanOrder found — treating as not fighting.")
            return False

        return pass4_scan <= max_scan_order <= end_scan
    except Exception:
        log.exception("[KVK CHECK] Unexpected failure in Pass4 gate")
        return False
    ...
    try:
        ...
        if hasattr(target, "date"):
            target = (
                ...
                else datetime.combine(target, datetime.min.time())
            )
...
async def _send_prekvk_embed(
    ...
    *,
    ...
    try:
        ...
        log.exception("[PREKVK] Honor block failed")
        honor_top = []
    ...
    if msg_id:
        try:
            ...
            if message and getattr(message, "created_at", None):
                if message.created_at.date() != today_utc:
                    message = None
    ...
    if message:
        try:
            ...
            log.exception("[PREKVK] Edit failed; will send a fresh message.")
    ...
    try:
        ...
        log.exception("[PREKVK] Failed to persist message id")
...
async def send_stats_update_embed(
    ...
    if effective_is_kvk:
        try:
            ...
            if state.pop("prekvk_msg_id", None) is not None:
                _save_state(state)
                log.info("[PREKVK] Fighting opened — cleared stored prekvk_msg_id.")
        except Exception:
            log.exception("[PREKVK] Failed to clear prekvk_msg_id on fighting-open.")
    ...
    if not channel:
        log.warning("⚠️ Could not find channel id %s.", channel_id)
        return
    ...
    if effective_is_kvk:
        ...
        if not is_test and _sent_today_any([KIND_OFFSEASON_DAILY, KIND_OFFSEASON_WEEKLY]):
            log.info("[STATS EMBED] Off-season already posted today; skipping KVK.")
            return

        # Respect daily cap (no log yet)
        if not is_test and _read_counts_for(KIND_KVK, _today_iso()) >= 3:
            log.info("[STATS EMBED] KVK daily limit reached, skipping broadcast.")
            return
        ...
        try:
            ...
            log.exception("[STATS EMBED] KVK send failed.")
        return
    ...
    if is_kvk and ENABLE_PREKVK:
        ...
        if not is_test and _sent_today_any([KIND_OFFSEASON_DAILY, KIND_OFFSEASON_WEEKLY]):
            log.info("[STATS EMBED] Off-season already posted today; skipping Pre-KVK.")
            return
        ...
        if not is_test and _sent_today(KIND_PREKVK_DAILY):
            log.info("[STATS EMBED] Pre-KVK already sent today; skipping.")
            return
        ...
        try:
            ...
            log.exception("[STATS EMBED] Pre-KVK send failed.")
        return
    ...
    if not is_test and _sent_today(KIND_KVK):
        log.info("[STATS EMBED] KVK already posted today; skipping off-season daily.")
        ...
        try:
            ...
            log.exception("[STATS EMBED] Off-season daily send failed.")
    else:
        log.info("[STATS EMBED] Off-season daily cooldown active; skipping.")

    # 2) Optional weekly post — only on Mondays, at most once on that Monday, no ping
    try:
        if datetime.now(UTC).weekday() == 0:  # Monday
            # Mutual exclusivity — if KVK already posted today, suppress off-season (weekly).
            if not is_test and _sent_today(KIND_KVK):
                log.info("[STATS EMBED] KVK already posted today; skipping off-season weekly.")
                ...
                try:
                    ...
                    log.exception("[STATS EMBED] Off-season weekly send failed.")
            else:
                log.info("[STATS EMBED] Off-season weekly already sent this Monday; skipping.")

    except Exception:
        log.exception("[STATS EMBED] Weekly off-season dashboard failed")
...
def _load_allkingdom_blocks(kvk_no: int) -> dict[str, list[dict]]:
    ...
    Returns dict with keys:
      players_by_kills / players_by_deads / players_by_dkp
      kingdoms_by_kills / kingdoms_by_deads / kingdoms_by_dkp
      camps_by_kills / camps_by_deads / camps_by_dkp
      our_top_player (list<=1), our_kingdom (list<=1), our_camp (list<=1)
    All metrics are aggregated across windows with StartScanID set.
    DKP is recomputed from aggregated T4/T5/Deads and proper denominators.
    """
    conn = pyodbc.connect(
        f"DRIVER={{ODBC Driver 17 for SQL Server}};"
        f"SERVER={SERVER};DATABASE={DATABASE};UID={USERNAME};PWD={PASSWORD};"
    ...
    with conn:
        ...
        # ---- Players (aggregate across fighting windows) ----
        # Weights (latest for this KVK)
        # Note: using CROSS APPLY-like subselects to keep code clear.
        players_sql = """
        WITH W AS (
          SELECT WindowName
          FROM KVK.KVK_Windows
          WHERE KVK_NO=? AND StartScanID IS NOT NULL
        ),
        Agg AS (
          SELECT p.governor_id, MAX(p.name) AS name, MAX(p.kingdom) AS kingdom, MAX(p.campid) AS campid,
                 SUM(p.t4_kills) AS t4, SUM(p.t5_kills) AS t5, SUM(p.deads) AS deads,
                 MAX(p.starting_power) AS sp
        ...
        SELECT TOP 3 name, kingdom, campid,
               (t4 + t5) AS kills_gain,
               deads,
               CAST(((t4*X + t5*Y + deads*Z) / NULLIF(sp,0)) AS float) AS dkp
        FROM Agg, Wt
        ...
        params = (kvk_no, kvk_no, kvk_no, kvk_no, kvk_no, kvk_no, kvk_no, kvk_no, kvk_no)
        c.execute(players_sql, params)
        p_kills = [
        ...
        # Our top 3 players in our kingdom
        c.execute(
            """
        WITH W AS (
          SELECT WindowName FROM KVK.KVK_Windows WHERE KVK_NO=? AND StartScanID IS NOT NULL
          ...
          SELECT p.governor_id,
                 ...
                 SUM(p.t4_kills + p.t5_kills) AS kills_gain
          FROM KVK.KVK_Player_Windowed p
          JOIN W ON W.WindowName = p.WindowName
        ...
        # ---- Kingdoms aggregate (proper DKP with denominator = sum of player SP per kingdom) ----
        kingdoms_sql = """
        WITH W AS (
          SELECT WindowName FROM KVK.KVK_Windows WHERE KVK_NO=? AND StartScanID IS NOT NULL
        ), Agg AS (
          SELECT kw.kingdom,
                 SUM(kw.t4_kills) AS t4, SUM(kw.t5_kills) AS t5, SUM(kw.deads) AS deads
          FROM KVK.KVK_Kingdom_Windowed kw
          JOIN W ON W.WindowName = kw.WindowName
          WHERE kw.KVK_NO = ?
          GROUP BY kw.kingdom
        ), Den AS (
          SELECT kingdom, SUM(sp) AS denom
          FROM (
            SELECT p.governor_id, MAX(p.kingdom) AS kingdom, MAX(p.starting_power) AS sp
            FROM KVK.KVK_Player_Windowed p
            WHERE p.KVK_NO = ?
            GROUP BY p.governor_id
          ) d
        ...
        SELECT TOP 3 a.kingdom, (a.t4 + a.t5) AS kills_gain, a.deads,
               CAST(((a.t4*X + a.t5*Y + a.deads*Z) / NULLIF(d.denom,0)) AS float) AS dkp
        FROM Agg a
        LEFT JOIN Den d ON d.kingdom = a.kingdom
        CROSS JOIN Wt
        ORDER BY a.deads DESC;
        ...
        ORDER BY dkp DESC;
        """
        kingdoms_params = (kvk_no, kvk_no, kvk_no, kvk_no) * 3
        c.execute(kingdoms_sql, kingdoms_params)
        k_kills = [
            dict(zip([d[0] for d in c.description], row, strict=False)) for row in c.fetchall()
        ]
        c.nextset()
        ...
        WITH W AS (SELECT WindowName FROM KVK.KVK_Windows WHERE KVK_NO=? AND StartScanID IS NOT NULL),
             Agg AS (
               SELECT SUM(t4_kills) AS t4, SUM(t5_kills) AS t5, SUM(deads) AS deads
               FROM KVK.KVK_Kingdom_Windowed kw
               JOIN W ON W.WindowName = kw.WindowName
             ...
             Den AS (
               ...
               FROM (
                 SELECT p.governor_id, MAX(p.starting_power) AS sp
                 FROM KVK.KVK_Player_Windowed p
                 WHERE p.KVK_NO=? AND p.kingdom=?
                 GROUP BY p.governor_id
               ) d
        ...
        WITH W AS (
          ...
          SELECT cw.campid, MAX(cw.camp_name) AS camp_name,
                 SUM(cw.t4_kills) AS t4, SUM(cw.t5_kills) AS t5, SUM(cw.deads) AS deads
          FROM KVK.KVK_Camp_Windowed cw
          JOIN W ON W.WindowName = cw.WindowName
          WHERE cw.KVK_NO = ?
          GROUP BY cw.campid
        ), Den AS (
          SELECT campid, SUM(sp) AS denom
          FROM (
            SELECT p.governor_id, MAX(p.campid) AS campid, MAX(p.starting_power) AS sp
            FROM KVK.KVK_Player_Windowed p
            WHERE p.KVK_NO = ?
            GROUP BY p.governor_id
          ) d
        ...
        SELECT TOP 3 a.campid, MAX(a.camp_name) AS camp_name,
               (a.t4 + a.t5) AS kills_gain, a.deads,
               CAST(((a.t4*X + a.t5*Y + a.deads*Z)/NULLIF(d.denom,0)) AS float) AS dkp
        FROM Agg a
        LEFT JOIN Den d ON d.campid = a.campid
        CROSS JOIN Wt
        GROUP BY a.campid, a.t4, a.t5, a.deads, d.denom, X, Y, Z
        ORDER BY kills_gain DESC;
        ...
        ORDER BY dkp DESC;
        """
        camps_params = (kvk_no, kvk_no, kvk_no, kvk_no) * 3
        c.execute(camps_sql, camps_params)
        c_kills = [
            dict(zip([d[0] for d in c.description], row, strict=False)) for row in c.fetchall()
        ]
        c.nextset()
    ...
    *,
    ...
    if thumb and thumb.lower().startswith(("http://", "https://")):
        try:
            ...
            log.exception("[KVK EMBED] Failed to set thumbnail")
    ...
    try:
        ...
        log.exception("[KVK EMBED] Honor block failed")

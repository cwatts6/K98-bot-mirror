...
def _to_ints(maybe_ids: list[str | int]) -> list[int]:
    out = []
    for v in maybe_ids:
        try:
...
def _is_single_day_slice(window_key: str) -> bool:
    # Treat these as single-day-only (yesterday == 1 day; adapt as needed)
    return window_key in {"yesterday"}
...
async def _fetch_proc(gov_ids: list[int], slices_csv: str, include_aggregate: bool) -> list[dict]:
    ...
    def _run():
        with _conn() as cn:
            cur = cn.cursor()
            cur.execute(sql, (_csv(gov_ids), slices_csv, 1 if include_aggregate else 0))
...
def _make_sparkline(
    ...
    if average is not None and len(ys) > 1:
        ...
        ax.text(xidx[-1], average, f" avg {_short(average)}", va="bottom", ha="left", fontsize=8)
    ...
    if "RSS" in (title or "") or y_max >= 10_000_000:
        ax.yaxis.set_major_formatter(FuncFormatter(lambda v, pos: _short(v)))
...
async def get_stats_payload(
    ...
    try:
        ...
        try:
            ...
            if trends and not _is_single_day_slice(slice_key):

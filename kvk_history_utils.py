...
# --- Project constants (adjust if your path/module differs)
...
def _configure_fonts_for_cjk():
    ...
    try:
        ...
        if _CJK_FAMILY is None:
            ...
            # Optional: silence just this noisy warning while we work without a CJK font
...
def get_started_kvks() -> list[int]:
    ...
    with _conn() as cn:
        with cn.cursor() as cur:
            ...
            row = cur.fetchone()
            max_started = int((row[0] if row else 0) or 0)
            ...
            if max_started == 0:
                ...
                r2 = cur.fetchone()
                max_started = int((r2[0] if r2 else 0) or 0)
            ...
            r3 = cur.fetchone()
            min_kvk = int(r3[0]) if r3 and r3[0] is not None else 3
...
# add this helper anywhere above build_dual_axis_chart
def _human_number(x: float) -> str:
    x = float(x or 0)
    absx = abs(x)
    if absx >= 1_000_000_000:
        v = x / 1_000_000_000.0
        return f"{v:.0f}b" if abs(v) >= 100 else (f"{v:.1f}b" if abs(v) >= 10 else f"{v:.2f}b")
    if absx >= 1_000_000:
        v = x / 1_000_000.0
        return f"{v:.0f}m" if abs(v) >= 100 else (f"{v:.1f}m" if abs(v) >= 10 else f"{v:.2f}m")
    if absx >= 1_000:
        v = x / 1_000.0
        return f"{v:.0f}k" if abs(v) >= 100 else (f"{v:.1f}k" if abs(v) >= 10 else f"{v:.2f}k")
    return f"{int(x):d}" if x.is_integer() else f"{x:.2f}"


def build_dual_axis_chart(
    df: pd.DataFrame,
    overlay: dict[int, str],  # <- accept overlay normally
    left_metrics: list[str],
    right_metric: str | None,
    title: str = "KVK History",
    show_point_labels: str = "none",  # "none" | "latest" | "all"
) -> io.BytesIO:
    if not left_metrics:
    ...
    ax_left.yaxis.set_major_formatter(FuncFormatter(lambda v, pos: _human_number(v)))
    ...
    if show_point_labels in ("latest", "all"):
        for gid, _label in overlay_safe.items():  # <- use overlay_safe here
            gdf = df[df["Gov_ID"] == gid].sort_values("KVK_NO")
            if gdf.empty:
                continue
            ...
            for mcol in left_cols:
                ...
                if show_point_labels == "latest" and xs_all:
                    _annotate_points(ax_left, [xs_all[-1]], [ys_all[-1]], _fmt_short)
                elif show_point_labels == "all":
                    _annotate_points(ax_left, xs_all, ys_all, _fmt_short)
...
def build_history_table_image(
    ...
    # Short number format to match the chart axes
    def _fmt_short(v):
        try:
            x = float(v)
        except Exception:

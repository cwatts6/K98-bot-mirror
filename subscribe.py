...
class SubscribeView(discord.ui.View):
    ...
    async def on_save(self, interaction: discord.Interaction):
        ...
        try:
            ...
            logger.exception("Failed to save subscription for user %s: %s", self.user.id, e)
            # User-visible failure - ephemeral
            await interaction.response.send_message(
                "⚠️ Failed to save your subscription due to an internal error. Please try again later.",
                ephemeral=True,
            )
            return

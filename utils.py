...
async def update_live_queue_embed(bot, notify_channel_id):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"[EMBED] Failed to send live queue embed after NotFound: {e}")
    except Exception as e:
        logger.error(f"[EMBED] Failed to send or update live queue embed: {e}")

    # Persist current jobs to disk using sync save_live_queue via run_blocking_in_thread
    try:

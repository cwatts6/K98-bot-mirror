...
def _is_retryable_gspread_details(details: dict) -> bool:
    ...
    try:
        ...
        pass
    if "unavailable" in repr_str.lower():
        return True
    return False
    ...
    *,
    ...
    while True:
        ...
        try:
            ...
            # SpreadsheetNotFound classified by _is_spreadsheet_not_found_transient()
            # (i.e. the HTTP response carried a 2xx status, indicating a Drive listing
            # pagination miss where the sheet exists but wasn't in the first page of
            # results) should be retried. Cases with an absent/None status_code are
            # NOT included — they are treated as genuine missing-sheet errors.
    ...
    def wrapped_transfer(*args, **kwargs):
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                details = _extract_api_error_details(ex)
                retryable = _is_retryable_gspread_details(details)
                # SpreadsheetNotFound with a 2xx HTTP response = Drive listing pagination miss → retryable.
                # A None/absent status_code is excluded by _is_spreadsheet_not_found_transient()
                # and is treated as a genuine missing-sheet error.
                if _is_spreadsheet_not_found_transient(ex):
                    retryable = True
...
def _aggregate_windowed_dfs(
    ...
    """
    Aggregate (sum) numeric columns across export sections filtered by WindowName values.
    - dfs: either a legacy positional list of result-set DataFrames, or a dict keyed by
      stable export section names
    - window_names: list of window display names to include (e.g., ["Pass 4", "1st Altar"])
    - src_idx: legacy positional index (int) or section reference/name (str) for the source
      section (e.g., player/kingdom/camp windowed data)
    ...
    try:
        if isinstance(dfs, dict):
            ...
            if section_name is None:
                return pd.DataFrame()
            df_section = dfs.get(section_name)
            if df_section is None:
                return pd.DataFrame()
            df_src = df_section.copy()
        else:

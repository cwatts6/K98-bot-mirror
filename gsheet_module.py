...
def check_basic_gsheets_access(
    ...
    """
    try:
        # Example: use existing client creation you have
    ...
    while attempt <= max_retries:
        try:
            ...
            # find a small, cheap read that confirms access
            ...
            logger.warning(
                "GSheets APIError (attempt %s/%s): %s", attempt, max_retries + 1, api_err
            )
            # If it's a 503 UNAVAILABLE, we should retry a couple times then fail
            # You can inspect api_err.response or str(api_err) for more detail
            if attempt > max_retries:
                return False, f"APIError after {attempt} attempts: {api_err}"

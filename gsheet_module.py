...
def _is_transient_error(exc: BaseException) -> bool:
    # common network/timeouts/ssl/http errors are transient
    ...
    txt = str(exc).lower()
    if (
        "decryption_failed_or_bad_record_mac" in txt
        or "eof occurred in violation of protocol" in txt
    ):
        return True
    return False
...
def run_all_exports(
    ...
    Returns a tuple (success: bool, log_messages: list[str]) for the run.
    """
    assert all(
    ...
    # Return success flag and collected logs to preserve compatibility with callers expecting a 2-tuple
...
def run_kvk_proc_exports(
    ...
    for df, tab in zip(dfs, _KVK_TABS_IN_ORDER, strict=False):
        ...
        # 🔧 ensure ALL datetime-like values are strings (other numerics preserved)
...
def create_additional_kvk_spreadsheets(
    ...
    comparison_tabs = []
    # mapping from metric -> tab base name
    level_map = {
        "kingdom": ("KINGDOM", 4),
        "camp": ("CAMP", 5),
        "player": ("PLAYER", 3),
    ...
    for metric in metrics:
        for agg_type, (label, src_idx) in level_map.items():
            tab_name = f"{label}_{metric.upper()}"
            comparison_tabs.append(
                {"tab_name": tab_name, "src_idx": src_idx, "agg_type": agg_type, "metric": metric}
            )
        ...
        try:
            ...
            for t, df_comp, has_data in comp_to_write:
                ...
                try:
                    ...
                    format_cols = []
                    # Reuse Player/Kingdom/Camp numeric hints where possible
                    if t["agg_type"] == "player":
                        ...
                        format_cols = _KVK_FORMAT_NUMBERS.get("KVK_Camp_Windowed", [])
                    # We add metric name too so numeric detection will pick it if present
                    if t["metric"] not in format_cols:
                        format_cols = format_cols + [t["metric"]]

                    # Filter the tentative format_cols to only those that actually exist in df_to_write (case-insensitive),
                    # to avoid spurious warnings and unnecessary attempts to format non-existent columns.
                    if format_cols:
                        df_cols_lower = {str(c).lower(): c for c in df_to_write.columns}
                        filtered_format_cols: list[Any] = []
                        for fc in format_cols:

...
from utils import load_live_queue, update_live_queue_embed, utcnow
...
async def _start_event_tasks_when_ready(max_wait_seconds: int = 300):
    ok = await wait_for_events(max_wait_seconds)
    if not ok:
        # one last quick check before giving up
    ...
    if ok:
        ...
        logger.warning(
            "[BOOT] Event cache still empty after %ss; skipping event-dependent tasks for now.",
            max_wait_seconds,
        )
...
async def _check_sql_health(timeout_sec: float = 3.0) -> tuple[bool, str | None]:
    ...
    int_timeout = int(timeout_sec)  # <-- ensure integer

    for driver in dsn_list:
        try:
            conn = pyodbc.connect(
            ...
            logger.warning("[HEALTH][SQL] Probe failed via '%s': %s", driver, last_err)
...
async def _check_gsheets_health(timeout_sec: float | None = None) -> tuple[bool, str]:
    """
    Use existing check_basic_gsheets_access(CREDENTIALS_FILE, SHEET_ID).
    Returns (ok, status_message) — message is short, safe for the embed.
    """
    ...
    try:
        ...
        if success:
            ...
            # Existing function already returns a human message
            return False, message[:160]
    except Exception as e:
        # Surface the actual exception (trim for embed)

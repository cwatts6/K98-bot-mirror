...
def _path_label(service, path_id: str, locale: str = "en-GB") -> str:
    ...
    try:
        ...
        logger.exception("[CrystalTech UI] _path_label failed")
        pass
    return path_id
...
class SetupView(discord.ui.View):
    ...
    async def _on_continue(self, interaction: discord.Interaction):
        ...
        try:
            # ensure_user_entry is async now; await it
            await service.ensure_user_entry(
        ...
        # Pass the interaction locale through if available for localized step names
        locale = getattr(interaction, "locale", "en-GB")
        view = ProgressView(
...
class ProgressView(discord.ui.View):
    ...
    def __init__(
        ...
        self._on_release = on_release
        self.message: discord.Message | None = None
        self.locale = locale or "en-GB"

        # Keep it simple: Save, Mark Next, Reset
        self.save_btn = discord.ui.Button(label="Save Progress", style=discord.ButtonStyle.success)
        self.mark_next_btn = discord.ui.Button(
            label="Mark next step complete", style=discord.ButtonStyle.primary
        )
        ...
        self.mark_next_btn.callback = self._on_mark_next
        self.reset_btn.callback = self._on_reset

        self.add_item(self.save_btn)
        self.add_item(self.mark_next_btn)
    ...
    async def render_embed(self) -> tuple[discord.Embed, list[discord.File]]:
        ...
        # Offload the synchronous computation to a thread to avoid blocking the event loop
        try:
            ns: NextSteps = await asyncio.to_thread(
                service.compute_next_steps, self.path_id, self.governor_id
            )
        except Exception:
            logger.exception("[CrystalTech UI] compute_next_steps failed for %s", self.governor_id)
            raise ValueError("Failed to compute CrystalTech progress for this path/account")

        title = "Crystal Tech — Progress"
        # Offload get_last_completed as it is synchronous
        try:
            last_step = await asyncio.to_thread(
                service.get_last_completed, self.path_id, self.governor_id
            )
        except Exception:
            logger.exception("[CrystalTech UI] get_last_completed failed for %s", self.governor_id)
            last_step = None

        path_pretty = _path_label(service, self.path_id)

        # Localized step names using the locale passed into the view
        def show_name(step):
            try:
                return step.display_name(self.locale)
            except Exception:
        ...
        desc = (
            ...
            f"**Last completed:** "
            f"{(show_name(last_step) + f' (#{last_step.order})') if last_step else 'none yet'}\n"
        ...
        if ns.next_two:
            ...
            for i, step in enumerate(ns.next_two):
                embed.add_field(
                    name=f"{labels[i]}: {show_name(step)} → Lv {step.target_level}",
        ...
        try:
            incomplete = await asyncio.to_thread(
                service.list_incomplete_steps, self.path_id, self.governor_id
            )
        except Exception:
            logger.exception(
                "[CrystalTech UI] list_incomplete_steps failed for %s", self.governor_id
            )
            incomplete = []
        ...
        opts = [
            discord.SelectOption(
                label=f"{show_name(s)} → Lv{s.target_level}",
                value=s.step_uid,
        ...
        if opts:
            ...
            self.mark_next_btn.disabled = False if ns.remaining_count > 0 else True
        else:
            self.save_btn.disabled = True
            self.mark_next_btn.disabled = True
        ...
        # inline image for Next 1 (attached and set as embed image)
        ...
        if ns.next_two:
            ...
            if os.path.isfile(img_path):
                try:
                    ...
                    files.append(f)
                    # embed image with attachment URL (shows inline inside embed)
                    embed.set_image(url=f"attachment://{f.filename}")
                except Exception:
                    logger.exception("[CrystalTech UI] failed to attach inline image %s", img_path)
    ...
    async def _on_save(self, interaction: discord.Interaction):
        ...
        current_entry = await asyncio.to_thread(service.get_user_entry, self.governor_id)
        ...
        # save_progress is async in the service
        try:
            ...
            logger.exception("[CrystalTech UI] save_progress failed for %s", self.governor_id)
            return await interaction.response.send_message(
                "Failed to save progress. Try again later.", ephemeral=True
            )
        ...
        try:
            ...
            # fallback: no files
            sent = await interaction.followup.send(embed=embed, ephemeral=True, view=new_view)
            new_view.message = sent
    ...
    async def _on_mark_next(self, interaction: discord.Interaction):
        if interaction.user.id != self.author_id:
            return await interaction.response.send_message(
                "This isn’t your session.", ephemeral=True
            )
        ...
        # Compute next steps and pick the very next step UID
        try:
            ns: NextSteps = await asyncio.to_thread(
                service.compute_next_steps, self.path_id, self.governor_id
            )
            next_step = ns.next_two[0] if ns.next_two else None
            if not next_step:
                return await interaction.response.send_message(
                    "No next step to mark — path may be complete.", ephemeral=True
                )
            step_uid = next_step.step_uid
        except Exception:
            logger.exception(
                "[CrystalTech UI] compute_next_steps failed for mark_next %s", self.governor_id
            )
            return await interaction.response.send_message(
                "Failed to determine next step. Try again later.", ephemeral=True
            )

        # Save the single next step (service.save_progress is async)
        try:
            await service.save_progress(
                governor_id=self.governor_id,
                path_id=self.path_id,
                troop_type=self.troop,
                newly_completed_uids=[step_uid],
            )
        except Exception:
            logger.exception(
                "[CrystalTech UI] save_progress failed for mark_next %s", self.governor_id
            )
            return await interaction.response.send_message(
                "Failed to mark step complete. Try again later.", ephemeral=True
            )

        # Refresh UI: freeze old view then open a fresh one
        self.disable_all_items()
        try:
            await interaction.response.edit_message(
                content="✅ Next step marked complete. Opening updated panel…",
                view=self,
                embed=None,
                attachments=[],
            )
...
class ResetConfirmView(discord.ui.View):
    ...
    async def confirm_btn(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        try:
            ...
            logger.exception(
                "[CrystalTech UI] reset_account_progress failed for %s", self.governor_id
            )
            return await interaction.response.send_message(
                "Failed to reset account. Try again later.", ephemeral=True
            )

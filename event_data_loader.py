...
# Try to import the centralized helper once. If unavailable, we will fallback to asyncio.to_thread()
try:
...
async def load_upcoming_ruins_events() -> list[Event] | None:
    """
    Fetch and parse 'ruins' timeline. Uses run_blocking_in_thread when available
    so telemetry/meta are recorded. Falls back to asyncio.to_thread if helper
    not present (flagged for conversion).
    """
    ...
    if run_blocking_in_thread is not None:
        return await run_blocking_in_thread(
            _load_upcoming_ruins_sync, name="load_upcoming_ruins_events", meta=meta
        )
    # TODO: convert this fallback to run_blocking_in_thread so telemetry/meta are captured.
    logger.debug(
...
async def load_upcoming_altar_events() -> list[Event] | None:
    ...
    if run_blocking_in_thread is not None:
        return await run_blocking_in_thread(
            _load_upcoming_altar_events_sync, name="load_upcoming_altar_events", meta=meta
        )
    logger.debug(
...
async def load_upcoming_major_events() -> list[Event] | None:
    ...
    if run_blocking_in_thread is not None:
        return await run_blocking_in_thread(
            _load_upcoming_major_events_sync, name="load_upcoming_major_events", meta=meta
        )
    logger.debug(
...
async def load_upcoming_chronicle_events() -> list[Event] | None:
    ...
    if run_blocking_in_thread is not None:
        return await run_blocking_in_thread(
            _load_upcoming_chronicle_events_sync, name="load_upcoming_chronicle_events", meta=meta
        )
    logger.debug(

...
from constants import SUBSCRIPTION_FILE
    ...
    try:
        if os.path.exists(SUBSCRIPTION_FILE):
            ...
            if not isinstance(data, dict):
                logger.warning("[SUBSCRIPTIONS] Invalid file schema; resetting to empty dict.")
                data = {}
            # Normalize entries
            norm: dict[str, dict] = {}
            ...
            subscriptions = norm
            logger.info(
                "[SUBSCRIPTIONS] Loaded subscription file (%d user(s)).", len(subscriptions)
            )
        else:
            subscriptions = {}
    except Exception as e:
        logger.error("[SUBSCRIPTIONS] Failed to load: %s", e)
        subscriptions = {}
...
def save_subscriptions() -> None:
    ...
    try:
        ...
        logger.info("[SUBSCRIPTIONS] Saved subscription file (%d user(s)).", len(subscriptions))
    except Exception as e:
        logger.error("[SUBSCRIPTIONS] Failed to save: %s", e)
        try:
...
def get_user_config(user_id) -> dict | None:
...
def update_user_reminder_times(user_id, times: list[str]) -> bool:
    _ensure_loaded()
    uid = str(user_id)
    if uid in subscriptions:  # type: ignore[union-attr]
...
def update_user_event_types(user_id, event_types: list[str]) -> bool:
    _ensure_loaded()
    uid = str(user_id)
    if uid in subscriptions:  # type: ignore[union-attr]
    ...
    *, dry_run: bool = True, keep_empty: bool = False
    ...
    if not dry_run:
        ...
        try:
            ...
            logger.warning("[SUBSCRIPTIONS] Backup failed: %s", e)

...
async def warm_target_cache() -> None:
    ...
    """
    try:
        # Local import to avoid module-level import cycles with file_utils
...
async def refresh_name_cache_from_sql() -> None:
    ...
    Offloads the actual DB work to a thread to avoid blocking the event loop.

    It reads from the view [dbo].[vw_All_Governors_Clean] which should expose
    GovernorID (int) and GovernorName (trimmed string).
    This implementation tries pandas.read_sql first then falls back to a DB cursor.
    """
    ...
    def _sync_refresh() -> None:
        try:
            ...
            logger.debug("[TARGET_UTILS] Starting SQL name cache refresh")
            sql = "SELECT [GovernorID], [GovernorName] FROM dbo.vw_All_Governors_Clean WHERE GovernorName IS NOT NULL"

            conn = _conn()
            use_pandas = True
            try:
                # pandas will prefer SQLAlchemy objects but works with some DBAPI connections
            ...
            if use_pandas:
                for _, r in df.iterrows():
                    ...
                    if norm not in norm_to_row:
                        row = {"GovernorName": original, "GovernorID": str(gov_id)}
                        norm_to_row[norm] = row
                        name_map[norm] = original
                    rows.append({"GovernorName": original, "GovernorID": str(gov_id)})
            else:
                cur = conn.cursor()
                try:
                    cur.execute(sql)
                    ...
                    for row in fetched:
                        ...
                        norm = _normalize_name(original)
                        if norm not in norm_to_row:
                            item = {"GovernorName": original, "GovernorID": str(gov_id)}
            ...
            logger.exception("[TARGET_UTILS] refresh_name_cache_from_sql sync worker failed")
            raise
    ...
    try:
        ...
        if str(query).strip().isdigit():
            ...
            try:
                ...
                tgt = await run_blocking_in_thread(
                ...
                class _KVKTargetsSelect(discord.ui.Select):
                    ...
                    async def callback(self, inter: discord.Interaction):
                        try:
                            ...
                            logger.exception("[TARGET_UTILS] _KVKTargetsSelect callback failed")
                            try:
                                await inter.followup.send(

...
def _load_existing_instance_ids_by_hash(conn) -> dict[bytes, list[int]]:
    cur = conn.cursor()
    cur.execute(
        "SELECT InstanceID, EffectiveHash FROM dbo.EventInstances WHERE EffectiveHash IS NOT NULL"
    )
    cols = [c[0] for c in cur.description]
    out: dict[bytes, list[int]] = {}
    for row in cur.fetchall():
        record = dict(zip(cols, row, strict=False))
        eff = record.get("EffectiveHash")
        instance_id = record.get("InstanceID")
        if eff is None or instance_id is None:
            continue
        out.setdefault(bytes(eff), []).append(int(instance_id))
    return out


def _insert_event_instance(
    cur, instance: dict[str, Any], *, instance_id: int | None = None
) -> None:
    if instance_id is None:
        ...
        """
        params = (
            instance.get("SourceKind"),
            instance.get("SourceID"),
            instance.get("StartUTC"),
            instance.get("EndUTC"),
            bool(instance.get("AllDay")),
            instance.get("Emoji"),
            instance.get("Title"),
            instance.get("EventType"),
            instance.get("Variant"),
            instance.get("Importance"),
            instance.get("Description"),
            instance.get("LinkURL"),
            instance.get("ChannelID"),
            instance.get("SignupURL"),
            instance.get("Tags"),
            instance.get("SortOrder"),
            bool(instance.get("IsCancelled")),
            instance.get("EffectiveHash"),
        )
    else:
        sql = """
        INSERT INTO dbo.EventInstances
        (InstanceID, SourceKind, SourceID, StartUTC, EndUTC, AllDay, Emoji, Title, EventType, Variant,
         Importance, Description, LinkURL, ChannelID, SignupURL, Tags, SortOrder, IsCancelled,
         GeneratedUTC, EffectiveHash)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, SYSUTCDATETIME(), ?)
        """
...
def write_event_instances(*, conn, instances: list[dict[str, Any]]) -> int:
    cur = conn.cursor()
    existing_ids_by_hash = _load_existing_instance_ids_by_hash(conn)

    for i in instances:
        i["EffectiveHash"] = compute_effective_hash(i)

    preserved_pairs: list[tuple[int, dict[str, Any]]] = []
    new_instances: list[dict[str, Any]] = []
    for instance in instances:
        reusable_ids = existing_ids_by_hash.get(bytes(instance["EffectiveHash"])) or []
        if reusable_ids:
            preserved_pairs.append((reusable_ids.pop(0), instance))
        else:
    ...
    if preserved_pairs:
        cur.execute("SET IDENTITY_INSERT dbo.EventInstances ON")
        for preserved_id, instance in preserved_pairs:
            _insert_event_instance(cur, instance, instance_id=preserved_id)
        cur.execute("SET IDENTITY_INSERT dbo.EventInstances OFF")

    for instance in new_instances:
        _insert_event_instance(cur, instance)

...
# ADD helper near other validators
...
def _detect_duplicate_ids(
    ...
    *,
    ...
    if dups:
        ...
        logger.error(
            "[CALENDAR][VALIDATION] duplicate_ids tab=%s id_field=%s ids=%s",
            tab,
            id_field,
            uniq,
        )
        raise ValueError(f"{tab}: duplicate {id_field} values: {uniq}")
...
def fetch_sheet_csv(sheet_id: str, tab: str, *, timeout: float = 60.0) -> list[dict[str, str]]:
    url = (
...
def parse_recurring_rules(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if not norm["RuleID"] or not norm["Title"] or not norm["EventType"]:
            logger.error(
                "[CALENDAR][VALIDATION] tab=recurring_rules rule_id=%s reason=missing_required_field",
                norm.get("RuleID"),
            )
        ...
        if norm["FirstStartUTC"] is None:
            logger.error(
                "[CALENDAR][VALIDATION] tab=recurring_rules rule_id=%s reason=missing_first_start_utc",
                norm.get("RuleID"),
            )
        ...
        if norm["DurationDays"] is None:
            logger.error(
                "[CALENDAR][VALIDATION] tab=recurring_rules rule_id=%s reason=missing_duration_days",
                norm.get("RuleID"),
            )
    ...
    _detect_duplicate_ids(out, id_field="RuleID", tab="recurring_rules")
    return out
...
def parse_oneoff_events(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if not norm["EventID"] or not norm["Title"] or not norm["EventType"]:
            logger.error(
                "[CALENDAR][VALIDATION] tab=oneoff_events event_id=%s reason=missing_required_field",
                norm.get("EventID"),
            )
            raise ValueError("oneoff_events: event_id/title/type required")
        if norm["StartUTC"] is None or norm["EndUTC"] is None:
            logger.error(
                "[CALENDAR][VALIDATION] tab=oneoff_events event_id=%s reason=missing_start_or_end_utc",
                norm.get("EventID"),
            )
            raise ValueError(f"oneoff_events[{norm['EventID']}]: start_utc/end_utc required")
        if norm["EndUTC"] <= norm["StartUTC"]:
            logger.error(
                "[CALENDAR][VALIDATION] tab=oneoff_events event_id=%s reason=end_before_start",
                norm.get("EventID"),
            )
    ...
    _detect_duplicate_ids(out, id_field="EventID", tab="oneoff_events")
    return out
...
def parse_overrides(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if not norm["OverrideID"] or not norm["TargetKind"] or not norm["TargetID"]:
            logger.error(
                "[CALENDAR][VALIDATION] tab=overrides override_id=%s reason=missing_required_field",
                norm.get("OverrideID"),
            )
            raise ValueError("overrides: override_id/target_kind/target_id required")
        if target_kind not in {"rule", "oneoff", "instance"}:
            logger.error(
                "[CALENDAR][VALIDATION] tab=overrides override_id=%s reason=rule_oneoff_instance",
                norm.get("OverrideID"),
            )
            raise ValueError(
        ...
        if action not in {"cancel", "modify"}:
            logger.error(
                "[CALENDAR][VALIDATION] tab=overrides override_id=%s reason=rule_cancel_modify",
                norm.get("OverrideID"),
            )
        ...
        if norm["NewEndUTC"] and norm["NewStartUTC"] and norm["NewEndUTC"] <= norm["NewStartUTC"]:
            logger.error(
                "[CALENDAR][VALIDATION] tab=overrides override_id=%s reason=end_before_start",
                norm.get("OverrideID"),
            )
            raise ValueError(
        ...
        norm["SourceRowHash"] = _hash_row(hash_input)
        out.append(norm)
    _detect_duplicate_ids(out, id_field="OverrideID", tab="overrides")
    return out

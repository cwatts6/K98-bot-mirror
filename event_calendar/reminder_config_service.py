...
def clear_user_calendar_reminder_preferences(
    user_id: int,
    *,
    writer: PrefsWriter = set_user_prefs,
) -> CalendarReminderMutationResult:
    ...
    try:
        ...
        logger.exception("calendar_reminder_preferences_clear_failed user_id=%s", user_id)
        return CalendarReminderMutationResult(
            ok=False,
            message="Failed to remove calendar reminders. Please try again in a moment.",
        )

    state = state_from_prefs(payload)
    logger.info("calendar_reminder_preferences_cleared user_id=%s", user_id)
    return CalendarReminderMutationResult(
        ok=True,
        message="Calendar reminders removed.",
        state=state,
        prefs=payload,
    )

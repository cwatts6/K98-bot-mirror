...
class CurrentRankingsMyRankAccountView(discord.ui.View):
    def __init__(
        ...
        *,
        ...
        ][:_MY_RANK_MAX_SELECT_ROWS]
        if len(choices) > _MY_RANK_SELECT_PAGE_SIZE * _MY_RANK_MAX_SELECT_ROWS:
            logger.warning(
                "kvk_current_rankings_my_rank_account_choices_truncated count=%s max=%s",
                len(choices),
                _MY_RANK_SELECT_PAGE_SIZE * _MY_RANK_MAX_SELECT_ROWS,
            )
...
class CurrentRankingsBrowserView(discord.ui.View):
    ...
    async def on_export_csv(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            row_label = "row" if export.row_count == 1 else "rows"
            await interaction.followup.send(
                content=f"CSV export ready: {export.row_count:,} ranked {row_label}.",

...
# Prefer the project's thread runner where available for better isolation.
try:
    ...
    if run_blocking_in_thread is not None:
        try:
            return await run_blocking_in_thread(
                fn, *args, name=name or getattr(fn, "__name__", None), meta=meta, **kwargs
            )
        except Exception:
            logger.debug(
                "run_blocking_in_thread failed for %r (name=%r); falling through to asyncio.to_thread",
                getattr(fn, "__name__", fn),
                name,
                exc_info=True,
            )

    # 2) fallback: asyncio.to_thread
...
def _extract_dataframe(maybe: Any) -> pd.DataFrame:
    ...
    return kvk_history_service.empty_history_frame()
...
class KVKHistoryView(discord.ui.View):
    ...
    def _selected_overlay_labels(self) -> dict[int, str]:
    ...
    def _build_chart_embed_only(
        ...
        highlights = []
        if not df.empty and "Gov_ID" in df.columns:
    ...
    async def _redraw_table(self, interaction: discord.Interaction):
        ...
        async with self._redraw_lock:
            ...
            # Prepare overlay map and reuse the same history df returned for the table.
            overlay_labels = self._selected_overlay_labels()

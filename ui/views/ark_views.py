...
class ArkAdminAddNameModal(discord.ui.Modal):
    def __init__(self, *, author_id: int, on_submit) -> None:
        super().__init__(title="Admin Add — Governor Lookup")
        self.author_id = author_id
        self._on_submit = on_submit
...
class ArkMatchResultModal(discord.ui.Modal):
    def __init__(
        self,
        *,
        author_id: int,
        ...
        current_notes: str,
        on_submit,
        result_label: str = "Result (Win/Loss)",
        modal_title: str = "Record Ark Result",
    ):
        super().__init__(title=modal_title, timeout=300.0)
        self.author_id = int(author_id)
        self._on_submit = on_submit

        input_cls = getattr(discord.ui, "TextInput", None) or getattr(discord.ui, "InputText", None)
        if input_cls is None:
            raise RuntimeError(
                "discord.ui TextInput/InputText is unavailable in this discord library build."
            )

        long_style = getattr(discord, "TextStyle", None)
        if long_style is not None:
            long_value = discord.TextStyle.long
        else:
            long_value = getattr(discord, "InputTextStyle", None).long

        self.result_input = input_cls(
            label=result_label,
            placeholder="Win or Loss",
            value=(current_result or "")[:20],
            required=True,
            max_length=20,
        )
        self.notes_input = input_cls(
            label="Notes (optional)",
            style=long_value,
            value=(current_notes or "")[:1000],
            required=False,
            max_length=1000,
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        result_val = getattr(self.result_input, "value", None)
        notes_val = getattr(self.notes_input, "value", None)

        # Normalize values
        result_str = str(result_val or "").strip()
        notes_str = str(notes_val or "").strip()
        normalized_notes = notes_str or None
    ...
    async def callback(self, interaction: discord.Interaction):
        """py-cord modal entry point; forwards to on_submit for compatibility."""
...
class ArkConfirmationView(LocalTimeToggleView):
    def __init__(
        ...
        *,
        ...
        on_record_result: Callable[[discord.Interaction], object],
        on_no_show: Callable[[discord.Interaction], object],
        on_create_teams: Callable[[discord.Interaction], object] | None = None,
        on_reconfirm_teams: Callable[[discord.Interaction], object] | None = None,
        show_check_in: bool = False,
        allow_emergency: bool = True,
        show_admin_actions: bool = False,
        show_result_actions: bool = False,
        teams_published: bool = False,
        timeout: float | None = None,
    ) -> None:
        super().__init__(
            events=[{"name": match_name, "start_time": match_datetime_utc}],
        ...
        self.on_record_result = on_record_result
        self.on_no_show = on_no_show
        self.on_create_teams = on_create_teams
        self.on_reconfirm_teams = on_reconfirm_teams
        ...
        if show_result_actions:
            self.result_btn = discord.ui.Button(
                label="Record Result",
                style=discord.ButtonStyle.primary,
                row=2,
                custom_id=f"{self.prefix}_record_result",
            )
            self.result_btn.callback = self._on_record_result
            self.add_item(self.result_btn)

            self.no_show_btn = discord.ui.Button(
                label="No Show",
                style=discord.ButtonStyle.secondary,
                row=2,
                custom_id=f"{self.prefix}_no_show",
            )
            self.no_show_btn.callback = self._on_no_show
            self.add_item(self.no_show_btn)

        if show_admin_actions:
            # "Confirm Teams" — always visible, disabled when teams are published
            self.create_teams_btn = discord.ui.Button(
                label="Confirm Teams",
                style=discord.ButtonStyle.blurple,
                row=2,
                custom_id=f"{self.prefix}_create_teams",
                disabled=teams_published,
            )
            self.create_teams_btn.callback = self._on_create_teams
            self.add_item(self.create_teams_btn)

            # "Reconfirm Teams" — only visible when teams are published
            if teams_published:
                self.reconfirm_teams_btn = discord.ui.Button(
                    label="Reconfirm Teams",
                    style=discord.ButtonStyle.secondary,
                    row=2,
                    custom_id=f"{self.prefix}_reconfirm_teams",
                )
                self.reconfirm_teams_btn.callback = self._on_reconfirm_teams
                self.add_item(self.reconfirm_teams_btn)
    ...
    async def _on_record_result(self, interaction: discord.Interaction):
        try:
            await self.on_record_result(interaction)
        except Exception:
            logger.exception("[ARK] Record result handler failed")
            await self._send_error_message(interaction, "❌ Failed to record result.")

    async def _on_no_show(self, interaction: discord.Interaction):
        try:
            await self.on_no_show(interaction)
        except Exception:
            logger.exception("[ARK] No-show handler failed")
            await self._send_error_message(interaction, "❌ Failed to mark no-show.")

    async def _on_create_teams(self, interaction: discord.Interaction):
        try:
            if self.on_create_teams:
                await self.on_create_teams(interaction)
        except Exception:
            logger.exception("[ARK] Create teams handler failed")
            await self._send_error_message(interaction, "❌ Failed to open team builder.")

    async def _on_reconfirm_teams(self, interaction: discord.Interaction):
        try:
            if self.on_reconfirm_teams:
                await self.on_reconfirm_teams(interaction)
        except Exception:
            logger.exception("[ARK] Reconfirm teams handler failed")
            await self._send_error_message(interaction, "❌ Failed to open team builder.")


class ArkResultNotesModal(discord.ui.Modal):
    def __init__(self, *, author_id: int, result: str, on_submit) -> None:
        super().__init__(title="Ark Result Notes")
        self.author_id = author_id
        self.result = result
        self._on_submit = on_submit
    ...
    async def callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.author_id:
            ...
            return
        notes = (self.children[0].value or "").strip()
        await self._on_submit(interaction, self.result, notes if notes else None)


class ArkResultSelectView(discord.ui.View):
    def __init__(self, *, author_id: int, on_submit) -> None:
        ...
        self.select = discord.ui.Select(
            ...
            options=[
                ...
                discord.SelectOption(label="Loss", value="Loss"),
            ],
            min_values=1,
            max_values=1,
        )
        ...
        self.notes_btn = discord.ui.Button(label="Add Notes", style=discord.ButtonStyle.secondary)
        self.notes_btn.callback = self._on_notes
        self.add_item(self.notes_btn)
    ...
    async def _on_select(self, interaction: discord.Interaction):
        ...
        await self._on_submit(interaction, result, None)

    async def _on_notes(self, interaction: discord.Interaction):
        if not self.select.values:
            await interaction.response.send_message("❌ Select Win or Loss first.", ephemeral=True)
            return

        async def _apply(inter: discord.Interaction, result_value: str, notes: str | None):
            await self._on_submit(inter, result_value, notes)

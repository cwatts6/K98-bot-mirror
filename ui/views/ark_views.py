...
class ArkMatchResultModal(discord.ui.Modal):
    def __init__(
        ...
        *,
        ...
        super().__init__(title="Record Ark Result")
        self.author_id = author_id
        self._on_submit = on_submit
    ...
    async def callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.author_id:
            ...
            return
        result = (self.children[0].value or "").strip()
        notes = (self.children[1].value or "").strip()
        await self._on_submit(interaction, result, notes if notes else None)

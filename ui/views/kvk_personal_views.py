...
class MyKVKStatsSelectView(discord.ui.View):
    ...
    - On select -> posts stats embeds via the channel fallback chain (commands.kvk_personal_posting)
    """
    ...
    async def _on_select(self, interaction: discord.Interaction):
        ...
        try:
            from commands.kvk_personal_posting import post_stats_embeds

            posted, _ = await post_stats_embeds(self._bot, self.ctx, embeds, file)
        except Exception:
    ...
    async def _on_register(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not summary.ok:
                raise RuntimeError(summary.error or "registry unavailable")
        except Exception as e:

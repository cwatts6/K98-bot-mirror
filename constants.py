...
def _env_float(name: str, default: float | None = None) -> float:
    ...
    if v is None:
        if default is None:
            raise RuntimeError(f"[CONFIG] Required float env var missing: {name}")
        return float(default)
    try:
        return float(v)
    except ValueError:
        raise RuntimeError(f"[CONFIG] {name} must be a float (got {v!r})")
...
EVENT_CALENDAR_CACHE_FILE_PATH = os.path.join(DATA_DIR, "event_calendar_cache.json")
EVENT_TYPE_INDEX_FILE_PATH = os.path.join(DATA_DIR, "event_type_index.json")
...
EVENT_CALENDAR_SHEET_ID = _env_str("EVENT_CALENDAR_SHEET_ID")  # optional
...
EVENT_CALENDAR_STALE_WARN_MINUTES = _env_int("EVENT_CALENDAR_STALE_WARN_MINUTES", 60)
EVENT_CALENDAR_STALE_DEGRADED_MINUTES = _env_int("EVENT_CALENDAR_STALE_DEGRADED_MINUTES", 240)
EVENT_CALENDAR_PIPELINE_INTERVAL_SECONDS = _env_int("EVENT_CALENDAR_PIPELINE_INTERVAL_SECONDS", 900)
EVENT_CALENDAR_PIPELINE_TIMEOUT_SECONDS = _env_int("EVENT_CALENDAR_PIPELINE_TIMEOUT_SECONDS", 120)

# EVENT CALENDAR ANOMALY GUARDRAILS (Task 5)
# Warn if publishable events unexpectedly drop to zero while prior non-zero state existed.
EVENT_CALENDAR_ANOMALY_ZERO_DROP_WARN = _env_bool("EVENT_CALENDAR_ANOMALY_ZERO_DROP_WARN", True)

EVENT_CALENDAR_ANOMALY_VOLUME_SPIKE_MULTIPLIER = _env_float(
    "EVENT_CALENDAR_ANOMALY_VOLUME_SPIKE_MULTIPLIER", 3.0
)

EVENT_CALENDAR_ANOMALY_CANCELLED_RATIO_WARN = _env_float(
    "EVENT_CALENDAR_ANOMALY_CANCELLED_RATIO_WARN", 0.60
)

# add Task-6 pipeline policy constants:
EVENT_CALENDAR_PIPELINE_RETRY_ENABLED = _env_bool("EVENT_CALENDAR_PIPELINE_RETRY_ENABLED", True)
EVENT_CALENDAR_PIPELINE_SYNC_TIMEOUT_SECONDS = _env_int(
    "EVENT_CALENDAR_PIPELINE_SYNC_TIMEOUT_SECONDS", 60
)
EVENT_CALENDAR_PIPELINE_GENERATE_TIMEOUT_SECONDS = _env_int(
    "EVENT_CALENDAR_PIPELINE_GENERATE_TIMEOUT_SECONDS", 90
)
EVENT_CALENDAR_PIPELINE_PUBLISH_TIMEOUT_SECONDS = _env_int(
    "EVENT_CALENDAR_PIPELINE_PUBLISH_TIMEOUT_SECONDS", 60
)
EVENT_CALENDAR_PIPELINE_SYNC_RETRIES = _env_int("EVENT_CALENDAR_PIPELINE_SYNC_RETRIES", 2)
EVENT_CALENDAR_PIPELINE_GENERATE_RETRIES = _env_int("EVENT_CALENDAR_PIPELINE_GENERATE_RETRIES", 1)
EVENT_CALENDAR_PIPELINE_PUBLISH_RETRIES = _env_int("EVENT_CALENDAR_PIPELINE_PUBLISH_RETRIES", 1)
EVENT_CALENDAR_PIPELINE_BACKOFF_BASE_SECONDS = _env_float(
    "EVENT_CALENDAR_PIPELINE_BACKOFF_BASE_SECONDS", 1.0
)
EVENT_CALENDAR_PIPELINE_BACKOFF_CAP_SECONDS = _env_float(
    "EVENT_CALENDAR_PIPELINE_BACKOFF_CAP_SECONDS", 10.0
)

EVENT_CALENDAR_REMINDER_GRACE_MINUTES = _env_int(
    "EVENT_CALENDAR_REMINDER_GRACE_MINUTES", default=15
)
EVENT_CALENDAR_REMINDER_LOOP_SECONDS = _env_int("EVENT_CALENDAR_REMINDER_LOOP_SECONDS", default=300)
...
def _require_gt_zero(name: str, value: int | float) -> None:
    if value <= 0:
        raise RuntimeError(f"[CONFIG] {name} must be > 0 (got {value!r})")


def _require_ge_zero(name: str, value: int | float) -> None:
    if value < 0:
        raise RuntimeError(f"[CONFIG] {name} must be >= 0 (got {value!r})")


def _validate_event_calendar_policy() -> None:
    # stale thresholds
    _require_ge_zero("EVENT_CALENDAR_STALE_WARN_MINUTES", EVENT_CALENDAR_STALE_WARN_MINUTES)
    _require_ge_zero("EVENT_CALENDAR_STALE_DEGRADED_MINUTES", EVENT_CALENDAR_STALE_DEGRADED_MINUTES)
    if EVENT_CALENDAR_STALE_DEGRADED_MINUTES < EVENT_CALENDAR_STALE_WARN_MINUTES:
        raise RuntimeError(
            "[CONFIG] EVENT_CALENDAR_STALE_DEGRADED_MINUTES must be >= EVENT_CALENDAR_STALE_WARN_MINUTES "
            f"(got {EVENT_CALENDAR_STALE_DEGRADED_MINUTES!r} < {EVENT_CALENDAR_STALE_WARN_MINUTES!r})"
        )

    # scheduler interval / legacy outer-timeout config
    _require_gt_zero(
        "EVENT_CALENDAR_PIPELINE_INTERVAL_SECONDS", EVENT_CALENDAR_PIPELINE_INTERVAL_SECONDS
    )
    _require_gt_zero(
        "EVENT_CALENDAR_PIPELINE_TIMEOUT_SECONDS", EVENT_CALENDAR_PIPELINE_TIMEOUT_SECONDS
    )
    ...
    if not (0.0 <= EVENT_CALENDAR_ANOMALY_CANCELLED_RATIO_WARN <= 1.0):
        raise RuntimeError(
            "[CONFIG] EVENT_CALENDAR_ANOMALY_CANCELLED_RATIO_WARN must be between 0.0 and 1.0 "
            f"(got {EVENT_CALENDAR_ANOMALY_CANCELLED_RATIO_WARN!r})"
        )
    ...
    _require_ge_zero("EVENT_CALENDAR_PIPELINE_SYNC_RETRIES", EVENT_CALENDAR_PIPELINE_SYNC_RETRIES)
    _require_ge_zero(
    ...
    if EVENT_CALENDAR_PIPELINE_BACKOFF_CAP_SECONDS < EVENT_CALENDAR_PIPELINE_BACKOFF_BASE_SECONDS:
        raise RuntimeError(
            "[CONFIG] EVENT_CALENDAR_PIPELINE_BACKOFF_CAP_SECONDS must be >= "
            "EVENT_CALENDAR_PIPELINE_BACKOFF_BASE_SECONDS "
            f"(got {EVENT_CALENDAR_PIPELINE_BACKOFF_CAP_SECONDS!r} < "
            f"{EVENT_CALENDAR_PIPELINE_BACKOFF_BASE_SECONDS!r})"
        )


_validate_event_calendar_policy()

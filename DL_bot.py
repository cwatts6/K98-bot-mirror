...
def _trigger_log_backup_sync(job_name: str = "ROK_TRACKER - LOG Backup") -> bool:
    """
    Best-effort synchronous trigger for a SQL Agent job that performs the log backup.

    Strategy:
    - Prefer calling the safe wrapper proc msdb.dbo.usp_start_rok_tracker_log_backup (least privilege).
    - If wrapper is not available, try job_id (env SQL_MONITOR_JOB_ID) then job_name using sp_start_job.
    - If sp_start_job fails, try ad-hoc BACKUP LOG to SQL_BACKUP_DIR.
...
# NEW: Async wrapper to call the sync trigger without blocking the loop

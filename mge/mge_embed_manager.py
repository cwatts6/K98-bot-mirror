...
async def sync_event_signup_embed(
    *,
    ...
    allowed_mentions = discord.AllowedMentions(
        everyone=bool(announce_everyone), roles=False, users=True
    )
    ...
    if msg_id:
        try:
            ...
            logger.info(
                "mge_embed_sync_updated event_id=%s message_id=%s channel_id=%s view_attached=%s",
                event_id,
                int(message.id),
                int(channel.id),
                view_attached,
    ...
    if message is None:
        try:
            ...
            view_attached = view is not None
            logger.info(
                "mge_embed_sync_created event_id=%s message_id=%s channel_id=%s view_attached=%s announce_everyone=%s is_rehydrate=%s",
                event_id,
                int(message.id),
                int(channel.id),
                view_attached,

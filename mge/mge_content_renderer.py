...
MAX_EMBED_FIELDS = 25
_OVERFLOW_FIELD_NAME = "⚠️ Overflow"
...
def render_legacy_mge_text(text: str, *, max_field_value: int = 1024) -> list[tuple[str, str]]:
    ...
    for idx, chunk in enumerate(chunks):
        name = "Rules" if idx == 0 else "Rules (cont.)"
    ...
    try:
        for section in sections:
            ...
            if len(result) >= MAX_EMBED_FIELDS - 1:
                ...
                result.append((_OVERFLOW_FIELD_NAME, "Content truncated — too many sections."))
                return result
...
def _truncate_embed_fields_with_overflow(
    fields: list[tuple[str, str]],
    *,
    fallback_name: str = "Rules",
) -> list[tuple[str, str]]:
    """Keep embed field lists within Discord's 25-field cap."""
    if not fields:
    ...
    truncated = fields[: MAX_EMBED_FIELDS - 1]
    truncated.append((_OVERFLOW_FIELD_NAME, "Content truncated — too many sections."))
    return truncated
...
def render_mge_content_to_embed_fields(
    ...
    *,
    ...
    try:
        ...
        fields = _truncate_embed_fields_with_overflow(fields, fallback_name=fallback_name)

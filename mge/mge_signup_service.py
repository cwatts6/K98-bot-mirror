...
from mge.dal import mge_roster_dal, mge_signup_dal
...
def _resolve_signup_discord_user_id(
    *,
    ...
    """
    Resolve Discord identity for signup persistence.

    - Self-service: preserve existing behavior (actor is the signup owner).
    - Admin add: resolve target governor owner from registry when present.
    """
    if not is_admin:
    ...
    if not owner:
        logger.info(
            "mge_admin_add_discord_identity_not_found event_governor_id=%s actor_discord_id=%s",
            governor_id,
            actor_discord_id,
        )
        return None
    ...
    logger.info(
        "mge_admin_add_discord_identity_resolved event_governor_id=%s resolved_discord_id=%s actor_discord_id=%s",
        governor_id,
        resolved_discord_id,
        actor_discord_id,
    )
...
def create_signup(
    *,
    ...
    signup_id = mge_signup_dal.insert_signup(
        ...
        discord_user_id=_resolve_signup_discord_user_id(
            is_admin=is_admin,
...
def withdraw_signup(
    *,
    ...
    award_cleanup = mge_roster_dal.delete_award_by_event_governor_with_audit_atomic(
        event_id=event_id,
        governor_id=governor_id,
        actor_discord_id=actor_discord_id,
        action_type="withdraw_cleanup",
        details={"signup_id": signup_id, "source": "signup_withdraw"},
        now_utc=now,
    )

    if award_cleanup is not None:
        logger.info(
            "mge_signup_service_withdraw_cleanup_award_deleted event_id=%s signup_id=%s governor_id=%s award_id=%s",
            event_id,
            signup_id,
            governor_id,
            award_cleanup.get("AwardId"),
        )

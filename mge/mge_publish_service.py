    ...
    *,
    ...
    for row in roster_rows:
        ...
        if award_id <= 0 or rank <= 0:
            continue
        target = max(rank1 - ((rank - 1) * 1_000_000), 0)
...
async def publish_event_awards(
    *,
    ...
    logger.info(
        "mge_publish_success event_id=%s actor_discord_id=%s publish_version=%s awarded=%s waitlist=%s",
        event_id,
        actor_discord_id,
        new_version,
        len(awarded),
    ...
    return PublishResult(True, "Published awards roster.", new_version, changes)

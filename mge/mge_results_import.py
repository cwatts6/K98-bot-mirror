    ...
    *,
    ...
    if isinstance(audit_context, dict):
        return MgeResultsImportAuditContext(
            ...
            actor_discord_id=_optional_int(audit_context.get("actor_discord_id", actor_discord_id)),
            event_id=_optional_int(audit_context.get("event_id", event_id)),
            event_mode=(
                str(audit_context["event_mode"]) if audit_context.get("event_mode") else event_mode
            ),
...
def import_results_auto(
    ...
    audit_ref = start_mge_results_audit_batch(context, content, source_file_hash_sha256=file_hash)

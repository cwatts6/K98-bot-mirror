...
def register_admin(bot: ext_commands.Bot) -> None:
    ...
    @bot.slash_command(
        ...
        description="Refresh Event Calendar from Google Sheets into SQL source tables.",
        guild_ids=[GUILD_ID],
    )
    @versioned("v1.01")
    @safe_command
    ...
    async def calendar_refresh(ctx: discord.ApplicationContext):
        ...
        async with get_operation_lock("calendar_refresh"):
            ...
            result = await svc.refresh(actor_user_id=ctx.user.id, sheet_id=sheet_id)

        color = discord.Color.green() if result.get("ok") else discord.Color.red()
        embed = discord.Embed(
            title="📅 Event Calendar Refresh",
            color=color,
        ...
        embed.add_field(name="Refreshed UTC", value=str(result.get("refreshed_utc")), inline=False)
        embed.add_field(
            name="Rows Read",
            value=(
                f"recurring={result.get('rows_read_recurring', 0)} | "
                f"oneoff={result.get('rows_read_oneoff', 0)} | "
                f"overrides={result.get('rows_read_overrides', 0)}"
            ),
            inline=False,
        )
        embed.add_field(
            name="Rows Upserted",
            value=(
                f"recurring={result.get('rows_upserted_recurring', 0)} | "
                f"oneoff={result.get('rows_upserted_oneoff', 0)} | "
                f"overrides={result.get('rows_upserted_overrides', 0)}"
            ),
            inline=False,
        )
    ...
    async def calendar_status(ctx: discord.ApplicationContext):
        ...
        embed = discord.Embed(
            ...
            color=0x2ECC71 if not health.get("current_degraded_mode") else 0xF39C12,
        )
        embed.add_field(
            name="Sync",
            value=(f"status: `{sync.get('status')}`\n" f"last: `{sync.get('last_refresh_utc')}`"),
            inline=False,
        )
        embed.add_field(
            name="Generate",
            value=(f"status: `{gen.get('status')}`\n" f"last: `{gen.get('last_generate_utc')}`"),
            inline=False,
        )
        embed.add_field(
            name="Publish",
            value=(f"status: `{pub.get('status')}`\n" f"last: `{pub.get('last_publish_utc')}`"),
            inline=False,
        )
        embed.add_field(
            name="Health",
            value=(
                f"cache_age_minutes: `{health.get('cache_age_minutes')}`\n"
                f"stale_warning: `{health.get('cache_stale_warning')}`\n"
                f"degraded: `{health.get('current_degraded_mode')}`\n"
                f"next_upcoming_event_utc: `{health.get('next_upcoming_event_utc')}`\n"
                f"last_successful_pipeline_utc: `{health.get('last_successful_pipeline_utc')}`"
            ),
            inline=False,
        )

...
from ark.ark_preference_service import ArkPreferenceError, clear_preference, set_preference
...
from ark.reminders import (
    cancel_match_reminders,
    dispatch_cancel_dms,
    reschedule_match_reminders,
)
...
async def _dispatch_cancel_dms_background(
    *, client, match_id: int, match: dict, roster: list
    ...
    """Fire cancel DMs in a background task so the interaction response is not blocked."""
    try:
...
def register_ark(bot: ext_commands.Bot) -> None:
        ...
        async def _on_confirm(interaction: discord.Interaction, sel):
            ...
            if existing_match:
                ...
                req = ArkMatchCreateRequest(
                    ...
                    match_day=match_day_short,
                    match_time_utc=match_time,
                    registration_starts_at_utc=datetime.now(UTC),
                    signup_close_utc=signup_close,
                    notes=None,
                    actor_discord_id=interaction.user.id,
                    calendar_instance_id=None,
            ...
            await interaction.response.edit_message(
                content=(
                    "✅ Match created"
                    f"{status_note}: Match ID `{match_id}`."
                    " Registration will open when the scheduled registration start time is reached."
                ),
                view=None,
            )
        ...
        async def _send_registration_result(
            ...
            *,
            ...
            await insert_audit_log(
                ...
                governor_id=None,
                details_json={"source": "CommandManual" if manual_match_id else "CommandDropdown"},
            )
    ...
    async def ark_cancel_match(ctx: discord.ApplicationContext):
        ...
        async def _on_confirm(interaction: discord.Interaction, sel):
            ...
            # ← REMOVED: the try/except await dispatch_cancel_dms block
            ...
            cancel_match_reminders(sel.match_id)
            ...
            # CR5: Acknowledge interaction first, then dispatch DMs in the background.
            # This prevents Discord's 3-second interaction timeout on larger rosters.
            await interaction.response.edit_message(
                content="✅ Match cancelled successfully.", view=None
            )

            # CR5: Fire DM dispatch as a background task — must be after edit_message
            asyncio.create_task(
    ...
    @channel_only(ARK_SETUP_CHANNEL_ID, admin_override=True)
    @track_usage()
    async def ark_set_preference(
        ctx: discord.ApplicationContext,
        governor_id: int = discord.Option(int, "Governor ID"),
        team: int = discord.Option(int, "Preferred team", choices=[1, 2]),
    ):
        await safe_defer(ctx, ephemeral=True)

        actor = f"discord:{ctx.user.id}"
        try:
        ...
        except ArkPreferenceError as exc:
            await ctx.followup.send(f"❌ {exc}", ephemeral=True)
            return
        except Exception:
            logger.exception("[ARK_PREF] Failed to set preference for governor_id=%s", governor_id)
            await ctx.followup.send("❌ Failed to save Ark team preference.", ephemeral=True)
            return

        governor_name = pref.get("GovernorName") or pref["GovernorID"]
        await ctx.followup.send(
            f"✅ Preference saved for **{governor_name}** (`{pref['GovernorID']}`) → Team **{pref['PreferredTeam']}**.",
            ephemeral=True,
        )
    ...
    @channel_only(ARK_SETUP_CHANNEL_ID, admin_override=True)
    @track_usage()
    async def ark_clear_preference(
        ctx: discord.ApplicationContext,
        governor_id: int = discord.Option(int, "Governor ID"),
    ):
        await safe_defer(ctx, ephemeral=True)
        ...
        actor = f"discord:{ctx.user.id}"
        try:
            cleared = await clear_preference(
                governor_id=governor_id,
                updated_by=actor,
            )
        except ArkPreferenceError as exc:
            ...
            logger.exception(
                "[ARK_PREF] Failed to clear preference for governor_id=%s", governor_id
            )
            await ctx.followup.send("❌ Failed to clear Ark team preference.", ephemeral=True)
            return

        if not cleared:
            await ctx.followup.send(
                f"ℹ️ No active preference was set for GovernorID `{governor_id}`.",
                ephemeral=True,
            )
            return

        await ctx.followup.send(
            f"✅ Cleared Ark team preference for GovernorID `{governor_id}`.",
            ephemeral=True,
        )
    ...
    @channel_only(ARK_SETUP_CHANNEL_ID, admin_override=True)
    @track_usage()
    async def ark_generate_draft(
        ctx: discord.ApplicationContext,
        match_id: int = discord.Option(int, "Ark match ID", min_value=1),
    ):
        await safe_defer(ctx, ephemeral=True)
        ...
        try:
            ...
            logger.exception("[ARK_CMD] /ark_generate_draft failed match_id=%s", int(match_id))
            await ctx.followup.send("❌ Failed to generate draft teams.", ephemeral=True)
            return

        await ctx.followup.send(
            (
                f"✅ Draft generated for Match `{int(match_id)}`\n"
                f"• Team 1: **{len(result.team1_ids)}** players, power **{result.team1_power:,}**\n"
                f"• Team 2: **{len(result.team2_ids)}** players, power **{result.team2_power:,}**\n"
                f"• Preference assigned: **{result.assigned_by_preference}**\n"
                f"• Balanced assigned: **{result.assigned_by_balancer}**"
            ),
            ephemeral=True,
        )

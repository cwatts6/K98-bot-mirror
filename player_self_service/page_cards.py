...
def _pack_summary_tokens(
    ...
    *,
    ...
    current = ""
    for token in tokens:
        token = _clean(token, fallback="").strip()
        if not token:
            continue
        ...
        if current:
            ...
            if len(lines) >= max_lines:
                return None
            current = token
        if _text_width(draw, current, font) > width:
            return None
    if current:
        lines.append(current)
...
def _summarize_items_for_width(
    ...
    *,
    ...
    for count in range(len(items), -1, -1):
        ...
        if count <= 0:
            tokens = (f"plus {remaining} more events",)
        else:
            tokens = items[:count]
            if remaining:
                tokens = (*tokens, f"plus {remaining} more events")
        lines = _pack_summary_tokens(
            draw,
            tokens,
            width=width,
            font=font,

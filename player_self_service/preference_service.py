    ...
    *,
    ...
    try:
        ...
        return PreferenceMutationResult(
            ok=False,
            message="Inventory visibility could not be saved. Please try again in a moment.",
        )

    if not result.ok or result.visibility != visibility:
        return PreferenceMutationResult(
            ok=False,
            message="Inventory visibility could not be saved. Your previous setting is unchanged.",
        )

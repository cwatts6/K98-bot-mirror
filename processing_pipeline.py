...
# NEW: lightweight post-import stats maintenance
try:
    import pyodbc
except Exception:
    pyodbc = None
...
def _run_post_import_stats_update(server, database, username, password, timeout_seconds=300):
    """
    Optional maintenance: refresh table/index statistics after large writes.
    Safe to skip on error; logs a warning and continues.
    """
    if pyodbc is None:
        logger.debug("[MAINT] pyodbc not available; skipping sp_updatestats.")
        return
    conn = None
    try:
        conn = pyodbc.connect(
...
async def execute_processing_pipeline(rank, seed, user, filename, channel_id, save_path=None):
    ...
    if success_sql:
        ...
        _run_post_import_stats_update(SERVER, DATABASE, USERNAME, PASSWORD)
        ...
        if success_sql:
            try:
                ...
                preflight_from_env_sync(
                    ...
                    wait_on_log_backup=True,  # critical: wait when LOG_BACKUP is detected
                    max_wait_seconds=150,  # 2.5 minutes is enough between 5-min agent cycles
                ...
                await send_embed(
                    ...
                    "🛠️ ProcConfig Import",
                    {
                        "Status": "Skipped (SQL log not ready)",
                        "Details": str(e),
                    },
                    0xE67E22,
                )

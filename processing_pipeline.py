...
from embed_utils import _DEFAULT_MAX_LOG_EMBED_CHARS, build_context_field, send_embed_safe
...
# ---- New helper: DRY status embed sender ----
async def _send_status_embed(
    ...
    *,
    ...
    - user, notify_channel: forwarded to send_embed_safe
    ...
    try:
        ...
        # embed_utils.send_embed_safe will attach and handle oversized logs, but we proactively trim the
        # combined log strings to avoid excessive memory in embed fields.
        if "Log" in fields and fields["Log"] is not None:
            try:
                fields["Log"] = str(fields["Log"])[:_EMBED_LOG_TRIM]
            except Exception:
                pass

        await send_embed_safe(user, title, fields, color, bot=bot, fallback_channel=notify_channel)

        # Emit a compact telemetry event for operational visibility
        telemetry_payload = {"event": "pipeline_status", "title": title, "ok": bool(ok)}
        ...
        logger.exception("[STATUS_EMBED] failed to send status embed")
...
async def execute_processing_pipeline(
    ...
    # Use DRY helper to send the Stats Copy Archive embed and telemetry
    await _send_status_embed(
        "✅ Stats Copy Archive",
        {
    ...
    if success_sql:
        ...
        try:
            ...
            if not ok:
                logger.exception(
                    "[MAINT] post-import stats update failed or timed out: %s", out[:4000]
                )
        ...
        if success_sql:
            try:
                ...
                try:
                    ...
                    if not ok:
                        logger.exception("[PROC_IMPORT] proc_import failed: %s", out[:4000])
                        emit_telemetry_event(
                            {
                                "event": "proc_import",
                                "status": "failed",

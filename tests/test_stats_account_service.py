...
def test_summarize_accounts_deduplicates_names_and_ids() -> None:
    """Duplicate GovernorIDs and names (e.g. same account in two slots) must not appear twice."""
    accounts = {
        "Main": {"GovernorID": "100", "GovernorName": "Shared"},
        "Alt 1": {"GovernorID": "100", "GovernorName": "Shared"},
        "Alt 2": {"GovernorID": "200", "GovernorName": "Other"},
    }

    summary = stats_account_service.summarize_accounts(accounts)

    assert summary.governor_ids == [100, 200]
    assert summary.account_names == ["Shared", "Other"]

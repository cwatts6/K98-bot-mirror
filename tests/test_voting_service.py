...
def test_build_create_request_validates_options_and_future_close():
    ...
    assert req.result_visibility == service.RESULT_VISIBILITY_PUBLIC_LIVE


def test_build_create_request_accepts_hidden_until_close_visibility():
    now = datetime(2026, 7, 1, 12, 0, tzinfo=UTC)
    ...
    req = service.build_create_request(
        ...
        reminder_mention_everyone=False,
        close_mention_everyone=False,
        result_visibility=service.RESULT_VISIBILITY_HIDDEN_UNTIL_CLOSE,
        now_utc=now,
    )
...
def test_build_create_request_rejects_unknown_result_visibility():
    ...
    with pytest.raises(service.VoteValidationError, match="result visibility"):
        service.build_create_request(
            guild_id=1,
            channel_id=2,
            created_by_discord_user_id=3,

...
def test_update_rules_rejects_embed_field_over_limit(monkeypatch):
    monkeypatch.setattr(
        "mge.mge_rules_service.mge_rules_dal.fetch_event_rules_context",
        lambda event_id: {"EventId": event_id, "RuleMode": "fixed", "RulesText": "old"},
    )

    res = mge_rules_service.update_event_rules_text(
        event_id=99,
        new_rules_text="x" * (mge_rules_service.RULES_EMBED_FIELD_LIMIT + 1),
        actor_discord_id=77,
    )

    assert res.success is False
    assert "actual:" in res.message
    assert "allowed:" in res.message
    assert "over by:" in res.message

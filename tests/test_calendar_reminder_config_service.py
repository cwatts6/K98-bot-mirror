...
def test_clear_user_calendar_reminder_preferences_disables_and_removes_buckets() -> None:
    calls = []

    result = service.clear_user_calendar_reminder_preferences(
        42,
        writer=lambda user_id, payload: calls.append((user_id, payload)),
    )
    ...
    assert result.state.selected_types == ()
    assert result.state.selected_offsets == ()
    assert calls == [(42, {"enabled": False, "by_event_type": {}})]

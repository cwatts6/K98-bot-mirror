...
async def test_final_day_goes_to_registration_channel(monkeypatch, tmp_path):
    ...
    reg_payloads = client.channels[111].sent
    reg_text = " ".join(
        ...
        for m in reg_payloads
    )

    conf_payloads = client.channels[222].sent
    conf_text = " ".join(
        str(m.get("content") or "").lower()
        + " "
        + str(getattr(m.get("embed"), "title", "") or "").lower()
        + " "
        + str(getattr(m.get("embed"), "description", "") or "").lower()
        for m in conf_payloads
    )

    assert any(token in reg_text for token in ("final_day", "final-day", "final day"))
    assert all(token not in conf_text for token in ("final_day", "final-day", "final day"))

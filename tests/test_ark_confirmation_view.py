...
def test_confirmation_view_hides_checkin_button():
    async def _case():
        view = ArkConfirmationView(
            ...
            on_record_result=lambda *_: None,
            on_no_show=lambda *_: None,
            show_check_in=False,
...
def test_confirmation_view_admin_buttons_only():
    async def _case():
        view = ArkConfirmationView(
            match_id=1,
            match_name="Ark Match — K98",
        ...
        labels = [getattr(c, "label", "") for c in view.children]
        assert "Record Result" in labels
        assert "No Show" in labels
        assert "Emergency — can't attend" not in labels

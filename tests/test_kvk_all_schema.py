...
def test_stage_insert_contract_includes_phase2_columns_and_metadata() -> None:
    ...
    assert required.issubset(set(importer.STAGE_PHASE2_REQUIRED_COLUMNS))
...
def test_ingest_returns_migration_order_error_when_stage_schema_is_outdated(
    ...
    class MockCursor:
        ...
        def fetchall(self) -> list[tuple[str]]:
            available_columns = set(importer.STAGE_INSERT_COLUMNS) - {
                "rank",
                "schema_version",
                "source_sheet_name",
            }
            return [(column,) for column in available_columns]
    ...
    assert result["success"] is False
    assert result["required_sql_migration"] == "sql/kvk_all_phase2_full_data_capacity.sql"
    assert result["missing_stage_columns"] == ["rank", "schema_version", "source_sheet_name"]
    assert "Apply sql/kvk_all_phase2_full_data_capacity.sql before deploying this importer." in (
        result["error"]
    )
    assert connection.cursor_obj.executemany_calls == []

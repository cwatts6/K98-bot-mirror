...
@pytest.fixture
def freeze_pre_start_now(monkeypatch):
    """
    Freeze now before post-start window for the fixed test match datetime
    (2026-03-07 11:00 UTC), so check-in/emergency actions are open.
    """
    monkeypatch.setattr(
        "ark.confirmation_flow.utcnow",
        lambda: datetime(2026, 3, 7, 10, 0, tzinfo=UTC),
    )
...
@pytest.mark.asyncio
async def test_check_in_marks_signup(monkeypatch, freeze_pre_start_now):
...
@pytest.mark.asyncio
async def test_check_in_prompts_for_governor_selection(monkeypatch, freeze_pre_start_now):
...
@pytest.mark.asyncio
async def test_emergency_withdraw_triggers_promotion(monkeypatch, freeze_pre_start_now):
...
@pytest.mark.asyncio
async def test_emergency_withdraw_updates_field(monkeypatch, freeze_pre_start_now):
...
@pytest.mark.asyncio
async def test_emergency_withdraw_prompts_for_governor_selection(monkeypatch, freeze_pre_start_now):
...
@pytest.mark.asyncio
async def test_record_result_uses_match_result_modal(monkeypatch):
    controller = ArkConfirmationController(match_id=1, config={"PlayersCap": 30, "SubsCap": 15})
    ...
    class DummyInteraction:
        def __init__(self):
            self.user = type("U", (), {"id": 123})()
            self.response = DummyResponse()

    async def _get_match(_mid):
        return {"MatchId": 1, "Status": "Locked", "Result": None, "ResultNotes": None}

    # ✅ allow modal flow in test
    monkeypatch.setattr("ark.confirmation_flow._is_admin_or_leadership", lambda *_a, **_k: True)
    monkeypatch.setattr("ark.confirmation_flow.get_match", _get_match)

    interaction = DummyInteraction()
    await controller.record_result(interaction)

    assert interaction.response.modal is not None
    assert interaction.response.modal.title == "Record Ark Result"

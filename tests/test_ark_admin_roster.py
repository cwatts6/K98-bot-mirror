...
@pytest.mark.asyncio
async def test_admin_add_partial_governor_id_fuzzy_matches(monkeypatch):
    controller = ArkRegistrationController(
        match_id=1,
    ...
    class DummyFuzzyView:
        ...
        async def send_followup(self, _interaction, _embed):
            self.sent.append({"interaction": _interaction, "embed": _embed})

    def _view_factory(matches, author_id, on_select):
        view = DummyFuzzyView(matches, author_id, on_select)
    ...
    ids = [m.get("GovernorID") for m in view.matches]
    assert "12072972" in ids

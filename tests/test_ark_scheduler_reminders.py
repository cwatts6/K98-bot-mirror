...
async def test_daily_channel_reminder_uses_registration_channel(monkeypatch, tmp_path):
    ...
    def _msg_contains_daily(msg: dict) -> bool:
        content = str(msg.get("content") or "").lower()
        emb = msg.get("embed")
        title = str(getattr(emb, "title", "") or "").lower()
        desc = str(getattr(emb, "description", "") or "").lower()
        return "daily" in content or "daily" in title or "daily" in desc

    assert any(_msg_contains_daily(m) for m in client.channels[111].sent)

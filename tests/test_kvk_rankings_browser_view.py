...
async def test_current_rankings_browser_export_csv_sends_private_file(monkeypatch):
    ...
    assert interaction.response.defer_kwargs == {"ephemeral": True}
    assert sent[0][0] == "CSV export ready: 1 ranked row."
    assert sent[0][1]["ephemeral"] is True
...
@pytest.mark.asyncio
async def test_current_rankings_my_rank_account_selector_truncates_above_discord_rows(caplog):
    choices = tuple(
        RankingAccountChoice(
            slot=f"Slot {index}",
            governor_id=1000 + index,
    ...
    caplog.set_level(logging.WARNING, logger=kvk_rankings_views.__name__)
    ...
    assert sum(len(select.options) for select in selects) == 125
    assert "account_choices_truncated" in caplog.text
...
def test_my_rank_embed_normalizes_governor_names():
    result = MyRankLookupResult(
        ...
        row_above=RankingRow(
            rank=11,
            governor_id=111,
            governor_name="Ahead\n`Name` @here <@456>",
            value=1200,
        ),
        row_below=RankingRow(
            rank=13,
            governor_id=222,
            governor_name="Behind\n`Name` @everyone <@789>",
            value=900,
        ),
        total_rows=50,
    )

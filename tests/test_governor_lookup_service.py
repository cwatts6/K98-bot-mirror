...
from services import governor_lookup_service as svc


@pytest.mark.asyncio
async def test_resolve_governor_query_exact_id(monkeypatch):
    ...
    result = await svc.resolve_governor_query("12072972")
...
@pytest.mark.asyncio
async def test_resolve_governor_query_partial_id_preserves_cache_order(monkeypatch):
    async def _ensure_ready():
        return None
    ...
    result = await svc.resolve_governor_query("120729")
...
@pytest.mark.asyncio
async def test_resolve_governor_query_partial_id_fetches_rows_once(monkeypatch):
    calls = {"rows": 0}
    ...
    monkeypatch.setattr(svc, "ensure_name_cache_ready", _ensure_ready)
    monkeypatch.setattr(svc, "get_name_cache_rows", _rows)
...
@pytest.mark.asyncio
async def test_resolve_governor_query_refreshes_and_uses_substring_fallback(monkeypatch):
    ...
    result = await svc.resolve_governor_query("tali")
...
@pytest.mark.asyncio
async def test_resolve_governor_query_preserves_numeric_no_match_message(monkeypatch):
    async def _ensure_ready():
        return None
    ...
    result = await svc.resolve_governor_query("123456")

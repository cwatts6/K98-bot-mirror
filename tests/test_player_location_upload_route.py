...
@pytest.mark.asyncio
async def test_player_location_route_refresh_failure_preserves_user_success(monkeypatch):
    rows = [(1, "A", 0, 0, 1, "TAG", 10, 20)]
    audit_calls = []
    ...
    async def _fail(batch_ref, **kwargs):
    ...
    monkeypatch.setattr(route, "parse_output_csv", lambda _bytes: rows)
    monkeypatch.setattr(route, "signal_location_refresh_complete", _signal)
    monkeypatch.setattr(route, "record_location_audit_phase", _record)
    monkeypatch.setattr(route, "complete_location_audit_batch", _complete)
    monkeypatch.setattr(route, "fail_location_audit_batch", _fail)
    ...
    assert handled is True
    assert len(sent) == 1
    _ch, _title, fields, color, mention = sent[0]
    assert fields["Imported Rows"] == "3"
    assert color == 0x2ECC71
    assert mention is None
    ...
    refresh_phase = next(
        ...
        if call[0] == "phase" and call[2]["phase_name"] == "location_post_import_refresh"
    )

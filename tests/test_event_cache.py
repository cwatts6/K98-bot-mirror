...
def test_refresh_overwrites_cache_if_stale_and_empty(monkeypatch):
    ...
    # Patch loaders to return empty lists (successful but empty)
    monkeypatch.setattr(edl, "load_upcoming_ruins_events", lambda: asyncio.sleep(0, result=[]))
    monkeypatch.setattr(edl, "load_upcoming_altar_events", lambda: asyncio.sleep(0, result=[]))
    monkeypatch.setattr(edl, "load_upcoming_major_events", lambda: asyncio.sleep(0, result=[]))
    monkeypatch.setattr(edl, "load_upcoming_chronicle_events", lambda: asyncio.sleep(0, result=[]))
...
def test_refresh_event_cache_times_out(monkeypatch, caplog):
    ...
    # Patch the loaders imported into event_cache module to our long/quick variants
    monkeypatch.setattr(

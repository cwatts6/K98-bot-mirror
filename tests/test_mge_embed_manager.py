...
def test_reminders_embed_respects_field_budget_when_cap_present() -> None:
    """Large reminders content must stay within Discord's 25-field limit."""
    from mge.mge_embed_manager import build_mge_award_reminders_embed

    event_row = {
        "EventName": "Big Reminder Event",
        "VariantName": "Infantry",
        "RuleMode": "fixed",
        "PointCapMillions": 8,
    }
    ...
    reminders_text = "\n\n".join(
        f"# Section {i}\n" + "\n".join(f"Line {i}.{j}" for j in range(3)) for i in range(30)
    )
    ...
    assert any(
        "overflow" in field.name.lower() or "truncated" in field.value.lower()

...
@pytest.mark.asyncio
async def test_stats_options_window_explains_format_and_timeframe() -> None:
    interaction = _Interaction()

    await export_views.send_stats_export_options(interaction, display_name="Tester")

    args, kwargs = interaction.response.sent[0]
    assert "Export type: Excel, CSV, or Google Sheets." in args[0]
    assert "Timeframe: last 90 days by default." in args[0]
    assert kwargs["ephemeral"] is True
    assert isinstance(kwargs["view"], export_views.StatsExportOptionsView)

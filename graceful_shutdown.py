        ...
        try:
            ...
            if cmdline and any("DL_bot.py" in str(arg) for arg in cmdline):
                ...
                try:
                    ...
                    await asyncio.sleep(0.5)  # ✓ Delay between kills
                except Exception as e:
...
def remove_shutdown_marker_if_allowed():
    ...
    if preserve:
        print("[SHUTDOWN] PRESERVE_SHUTDOWN_MARKER=1; leaving marker file in place.")
        return

    try:
        if os.path.exists(SHUTDOWN_MARKER_FILE):
            os.remove(SHUTDOWN_MARKER_FILE)
            print(f"[SHUTDOWN] Removed shutdown marker file: {SHUTDOWN_MARKER_FILE}")
    except Exception as e:
        print(f"[SHUTDOWN] Failed to remove shutdown marker file: {e}")
...
async def main():
    ...
    # Remove the persistent shutdown marker so restarted bots won't immediately
    # interpret it as a shutdown flag and re-shutdown. Set PRESERVE_SHUTDOWN_MARKER=1
    # in the environment to keep the old behavior.
